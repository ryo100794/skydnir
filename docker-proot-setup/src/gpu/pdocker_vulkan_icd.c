...
static bool pdocker_vk_resolve_image_executor_eligible(const PdockerVkImageResolveOp *op) {
    ...
    if (op->src->format != op->dst->format ||
        ...
        (op->src_layout != VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL &&
         op->src_layout != VK_IMAGE_LAYOUT_GENERAL) ||
        (op->dst_layout != VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL &&
         op->dst_layout != VK_IMAGE_LAYOUT_GENERAL)) {
        return false;
    }
    if (pdocker_vk_format_is_depth_stencil(op->src->format) ||
    ...
    if (op->region.srcSubresource.baseArrayLayer >= op->src->array_layers ||
        op->region.srcSubresource.layerCount >
            op->src->array_layers - op->region.srcSubresource.baseArrayLayer ||
        op->region.dstSubresource.baseArrayLayer >= op->dst->array_layers ||
        op->region.dstSubresource.layerCount >
            op->dst->array_layers - op->region.dstSubresource.baseArrayLayer ||
        op->region.srcOffset.x < 0 || op->region.srcOffset.y < 0 || op->region.srcOffset.z < 0 ||
        op->region.dstOffset.x < 0 || op->region.dstOffset.y < 0 || op->region.dstOffset.z < 0 ||
        (uint32_t)op->region.srcOffset.x > src_extent.width ||
        op->region.extent.width > src_extent.width - (uint32_t)op->region.srcOffset.x ||
        (uint32_t)op->region.srcOffset.y > src_extent.height ||
        op->region.extent.height > src_extent.height - (uint32_t)op->region.srcOffset.y ||
        (uint32_t)op->region.srcOffset.z > src_extent.depth ||
        op->region.extent.depth > src_extent.depth - (uint32_t)op->region.srcOffset.z ||
        (uint32_t)op->region.dstOffset.x > dst_extent.width ||
        op->region.extent.width > dst_extent.width - (uint32_t)op->region.dstOffset.x ||
        (uint32_t)op->region.dstOffset.y > dst_extent.height ||
        op->region.extent.height > dst_extent.height - (uint32_t)op->region.dstOffset.y ||
        (uint32_t)op->region.dstOffset.z > dst_extent.depth ||
        op->region.extent.depth > dst_extent.depth - (uint32_t)op->region.dstOffset.z) {
        return false;
    }
    return true;
}

static bool pdocker_vk_resolve_image_host_fallback_eligible(const PdockerVkImageResolveOp *op) {
    /*
     * Vulkan image resolve is sample reduction, not a byte copy.  Keep the
     * host fallback closed; eligible resolves must go through the native
     * executor path so multisample semantics stay owned by the real Vulkan
     * driver.
     */
    (void)op;
    return false;
...
static bool command_op_sequence_in_range(
        uint32_t sequence,
        uint32_t sequence_begin,
        uint32_t sequence_end) {
    return sequence >= sequence_begin && sequence < sequence_end;
}

static bool graphics_record_in_sequence_range(
        const PdockerVkGraphicsCommandRecord *record,
        uint32_t sequence_begin,
        uint32_t sequence_end) {
    return record && command_op_sequence_in_range(
        record->command_op_sequence, sequence_begin, sequence_end);
}

static bool graphics_record_is_state_preamble_command(uint32_t command_type) {
    switch (command_type) {
...
static bool graphics_record_should_serialize_for_range(
        const PdockerVkGraphicsCommandRecord *record,
        uint32_t sequence_begin,
        uint32_t sequence_end,
        bool include_state_preamble) {
    if (!record) return false;
    if (graphics_record_in_sequence_range(record, sequence_begin, sequence_end)) {
        return true;
    }
    return include_state_preamble &&
           record->command_op_sequence < sequence_begin &&
           graphics_record_is_state_preamble_command(record->command_type);
}

static bool command_buffer_has_graphics_records_in_sequence_range(
        const PdockerVkCommandBuffer *cmd,
        uint32_t sequence_begin,
        uint32_t sequence_end) {
    if (!cmd || sequence_begin > sequence_end) return false;
    for (uint32_t i = 0; i < cmd->graphics_command_op_count; ++i) {
        if (graphics_record_in_sequence_range(&cmd->graphics_command_ops[i],
...
static bool command_op_requires_graphics_executor_frame(
        const PdockerVkCommandBuffer *cmd,
        uint32_t op_index) {
    if (!cmd || op_index >= cmd->command_op_count) return false;
    const PdockerVkCommandOp *op = &cmd->command_ops[op_index];
    switch (op->type) {
        case PDOCKER_VK_COMMAND_RESOLVE_IMAGE:
...
static bool command_buffer_has_executor_frame_content_in_sequence_range(
        const PdockerVkCommandBuffer *cmd,
        uint32_t sequence_begin,
        uint32_t sequence_end) {
    if (!cmd || sequence_begin > sequence_end) return false;
    if (command_buffer_has_graphics_records_in_sequence_range(cmd, sequence_begin, sequence_end)) {
        return true;
    }
    for (uint32_t op_index = 0; op_index < cmd->command_op_count; ++op_index) {
        if (!command_op_sequence_in_range(op_index, sequence_begin, sequence_end)) continue;
        if (command_op_requires_graphics_executor_frame(cmd, op_index)) {
            return true;
        }
    }
    return false;
...
static int send_recorded_vulkan_graphics_v6_1_frame_range(
    ...
    size_t submit_info_count = 0;
    size_t submit_sync_info_count = 0;
    if (g_submit2_metadata_override) {
        submit_infos[0].submit_kind = PDOCKER_GPU_GRAPHICS_V621_SUBMIT_KIND_SUBMIT2;
        submit_infos[0].submit_flags = g_submit2_metadata_override->flags;
        submit_infos[0].command_buffer_index = g_submit2_metadata_command_index;
        submit_infos[0].command_buffer_device_mask = 0;
        if (g_submit2_metadata_command_index != PDOCKER_GPU_GRAPHICS_V621_COMMAND_BUFFER_INDEX_NONE &&
            g_submit2_metadata_override->pCommandBufferInfos &&
            g_submit2_metadata_command_index < g_submit2_metadata_override->commandBufferInfoCount) {
            submit_infos[0].command_buffer_device_mask =
    ...
    const char *graphics_label = "VULKAN_GRAPHICS_V6.1";
    {
        for (uint32_t gi = 0; gi < cmd->graphics_command_op_count; ++gi) {
            const PdockerVkGraphicsCommandRecord *record = &cmd->graphics_command_ops[gi];
            if (!graphics_record_should_serialize_for_range(
                    record, sequence_begin, sequence_end, include_state_preamble)) continue;
            if (record->command_type == PDOCKER_GPU_GRAPHICS_V6_COMMAND_CLEAR_ATTACHMENTS) {
                pre_need_v616_clear_attachments = true;
            }
            if (record->command_type == PDOCKER_GPU_GRAPHICS_V6_COMMAND_RESET_QUERY_POOL ||
            ...
            if (record->command_type == PDOCKER_GPU_GRAPHICS_V6_COMMAND_COPY_QUERY_POOL_RESULTS) {
                pre_need_v617_query = true;
                pre_need_v618_copy_query = true;
            }
            if ((pre_need_v616_clear_attachments && pre_need_v617_query && pre_need_v618_copy_query) || need_v619_submit_sync) {
                break;
            }
        }
        for (uint32_t op_index = 0; op_index < cmd->command_op_count; ++op_index) {
    ...
    PdockerGpuVulkanGraphicsV621FrameHeader *frame_header_v621 =
        (PdockerGpuVulkanGraphicsV621FrameHeader *)frame;
    ...
    for (uint32_t i = 0; i < cmd->graphics_command_op_count; ++i) {
        ...
        uint32_t multisample_flags = 0;
        if (pipeline->sample_shading_enable) {
            multisample_flags |= PDOCKER_GPU_GRAPHICS_V622_MULTISAMPLE_SAMPLE_SHADING_ENABLE;
        }
        if (pipeline->sample_mask_word_count > 0) {
            multisample_flags |= PDOCKER_GPU_GRAPHICS_V622_MULTISAMPLE_SAMPLE_MASK_PRESENT;
        }
        if (pipeline->alpha_to_coverage_enable) {
        ...
        if (multisample_flags != 0 || pipeline->min_sample_shading != 0.0f) {
            if (multisample_state_count >= PDOCKER_GPU_VULKAN_GRAPHICS_V622_MAX_MULTISAMPLE_STATES) {
                rc = -E2BIG;
                goto cleanup;
            }
            PdockerGpuVulkanGraphicsV622MultisampleStateEntry *ms =
                &multisample_states[multisample_state_count++];
            ms->pipeline_index = (uint32_t)pipeline_count;
            ms->flags = multisample_flags;
    ...
    uint32_t next_command_op_for_graphics = sequence_begin;
#define APPEND_INTERLEAVED_GRAPHICS_BUFFER_COPIES(until_sequence_) \
    do { \
        ...
        while (next_command_op_for_graphics < cmd->command_op_count && \
               next_command_op_for_graphics < sequence_end && \
...
#undef APPEND_INTERLEAVED_GRAPHICS_BUFFER_COPIES
    ...
    if (need_v621_submit2_metadata) {
        /* V6.21 is append-only after V6.20, so populate V6.19/V6.20 prefixes too. */
        need_v620_image_layout_range = true;
        need_v619_submit_sync = true;
    } else if (need_v620_image_layout_range) {
    ...
    if (need_v623_tessellation_state) {
        ...
        header->header_size = sizeof(*frame_header_v621);
    ...
    if (!need_v611_buffer_write && !need_v612_clear_color &&
        ...
        !need_v617_query && !need_v618_copy_query && !need_v619_submit_sync) {
    ...
    if (need_v619_submit_sync) {
        frame_header_v619->v619.submit_sync_count = (uint32_t)submit_sync_count;
    ...
    if (need_v621_submit2_metadata) {
        frame_header_v621->v621.submit_info_count = (uint32_t)submit_info_count;
        frame_header_v621->v621.submit_info_entry_size = sizeof(PdockerGpuVulkanGraphicsV621SubmitInfoEntry);
        frame_header_v621->v621.submit_info_schema_hash = PDOCKER_GPU_VULKAN_GRAPHICS_V621_SUBMIT_INFO_SCHEMA_HASH;
        frame_header_v621->v621.submit_sync_info_count = (uint32_t)submit_sync_info_count;
        frame_header_v621->v621.submit_sync_info_entry_size = sizeof(PdockerGpuVulkanGraphicsV621SubmitSyncInfoEntry);
        frame_header_v621->v621.submit_sync_info_schema_hash = PDOCKER_GPU_VULKAN_GRAPHICS_V621_SUBMIT_SYNC_INFO_SCHEMA_HASH;
    }
    if (need_v622_multisample_state) {
        frame_header_v622->v622.multisample_state_count = (uint32_t)multisample_state_count;
...
#define APPEND_GRAPHICS_TABLE(data_, count_, entry_size_, offset_field_, size_field_) \
    ...
    if (need_v621_submit2_metadata) {
        APPEND_GRAPHICS_TABLE(submit_infos, submit_info_count,
                              sizeof(submit_infos[0]),
                              frame_header_v621->v621.submit_info_table_offset,
                              frame_header_v621->v621.submit_info_table_size);
        APPEND_GRAPHICS_TABLE(submit_sync_infos, submit_sync_info_count,
                              sizeof(submit_sync_infos[0]),
                              frame_header_v621->v621.submit_sync_info_table_offset,
                              frame_header_v621->v621.submit_sync_info_table_size);
    }
    if (need_v622_multisample_state) {
        APPEND_GRAPHICS_TABLE(multisample_states, multisample_state_count,
                              sizeof(multisample_states[0]),
...
static void execute_recorded_resolve_image_op(
        ...
        PdockerVkCopyStats *stats) {
    if (!pdocker_vk_resolve_image_host_fallback_eligible(op) ||
        op->region.srcSubresource.aspectMask != VK_IMAGE_ASPECT_COLOR_BIT ||
        op->region.dstSubresource.aspectMask != VK_IMAGE_ASPECT_COLOR_BIT ||
        op->region.srcSubresource.layerCount == 0 ||
        op->region.srcSubresource.layerCount != op->region.dstSubresource.layerCount ||
...
static bool capture_single_subpass_dependency(
    ...
    if (dst_subpass != VK_SUBPASS_EXTERNAL && src_subpass + 1u == dst_subpass) {
        merge_render_pass_dependency_state(&rp->subpass_dependencies[dst_subpass],
                                           src_stage_mask, src_access_mask,
                                           dst_stage_mask, dst_access_mask);
        return true;
    }
    ...
    for (uint32_t i = 0; i < createInfoCount; ++i) {
        ...
        if (ci->pMultisampleState) {
            ...
            if (ms->pSampleMask) {
                ...
                if (words > PDOCKER_GPU_VULKAN_GRAPHICS_V622_MAX_SAMPLE_MASK_WORDS) {
                    pipeline->graphics_unsupported = true;
                } else {
                    pipeline->sample_mask_word_count = words;
                    for (uint32_t w = 0; w < words; ++w) {
                        pipeline->sample_mask_words[w] = ms->pSampleMask[w];
                    }
                }
            }
        }
...
static bool append_render_pass_subpass_layout_transitions(
        PdockerVkCommandBuffer *cmd,
        uint32_t subpass_index,
        const PdockerVkSubpassDependencyState *dependency) {
    if (!cmd || !cmd->active_render_pass || !cmd->active_framebuffer ||
        subpass_index >= PDOCKER_VK_MAX_STORAGE_BUFFERS) return false;
    PdockerVkRenderPass *rp = cmd->active_render_pass;
    const PdockerVkSubpassState *subpass = &rp->subpasses[subpass_index];
    uint32_t memory_first = cmd->memory_barrier_op_count;
    if (dependency && dependency->seen) {
        record_memory_barrier_op((VkCommandBuffer)cmd,
                                 render_pass_nonzero_access_mask(dependency->src_access_mask),
                                 render_pass_nonzero_access_mask(dependency->dst_access_mask),
                                 render_pass_nonzero_stage_mask(dependency->src_stage_mask),
                                 render_pass_nonzero_stage_mask(dependency->dst_stage_mask));
    }
        ...
        if (attachment->image_view && color_index < rp->attachment_count) {
            ...
            if (!record_render_pass_attachment_transition(
                    ...
                    render_pass_attachment_access_mask(false, false) |
                        (dependency && dependency->seen ? dependency->dst_access_mask : 0),
                    dependency && dependency->seen
                        ? render_pass_nonzero_stage_mask(dependency->src_stage_mask)
                        : render_pass_begin_src_stage_mask(old_layout),
                    render_pass_attachment_stage_mask(false) |
                        (dependency && dependency->seen ? dependency->dst_stage_mask : 0))) {
                return false;
            }
    ...
    if (subpass->has_depth_stencil_attachment &&
        ...
        VkImageLayout old_layout = cmd->active_render_pass_attachment_seen[ds_index]
        ...
        if (subpass->has_depth_stencil_resolve_attachment &&
            ...
            if (resolve_view) {
                ...
                if (!record_render_pass_attachment_transition(
                        ...
                        dependency && dependency->seen
                            ? render_pass_nonzero_access_mask(dependency->src_access_mask)
                            : render_pass_begin_src_access_mask(resolve_old_layout),
                        VK_ACCESS_2_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT |
                            (dependency && dependency->seen ? dependency->dst_access_mask : 0),
                        dependency && dependency->seen
                            ? render_pass_nonzero_stage_mask(dependency->src_stage_mask)
                            : render_pass_begin_src_stage_mask(resolve_old_layout),
                        render_pass_attachment_stage_mask(true) |
                            (dependency && dependency->seen ? dependency->dst_stage_mask : 0))) {
                    return false;
                }
...
static bool append_render_pass_final_layout_transitions(PdockerVkCommandBuffer *cmd) {
    ...
    for (uint32_t a = 0; a < rp->attachment_count && a < PDOCKER_VK_MAX_STORAGE_BUFFERS; ++a) {
        ...
        if (!record_render_pass_attachment_transition(
                ...
                render_pass_layout_stage_mask(old_layout),
                render_pass_layout_stage_mask(final_layout))) {
            return false;
        }
        cmd->active_render_pass_attachment_layouts[a] = final_layout;
    }
    return append_graphics_barrier_record_for_ranges(
        cmd, memory_first, cmd->memory_barrier_op_count - memory_first,
        first, cmd->image_barrier_op_count - first);
    ...
    for (uint32_t c = 0; c < subpass->color_attachment_count; ++c) {
        if (subpass->color_attachments[c] != VK_ATTACHMENT_UNUSED) {
            if (!populate_render_pass_attachment_for_rendering(
                    ...
                    clear_values, clear_value_count, false)) {
                return false;
            }
        }
    ...
    if (subpass->has_depth_stencil_attachment) {
        ...
        if (pdocker_vk_format_has_depth(ds_format) &&
            !populate_render_pass_attachment_for_rendering(
                ...
                subpass->depth_stencil_layout, clear_values, clear_value_count, false)) {
            return false;
        }
        if (pdocker_vk_format_has_stencil(ds_format) &&
            !populate_render_pass_attachment_for_rendering(
                &cmd->active_stencil_attachment, rp, fb, ds_index,
                subpass->depth_stencil_layout, clear_values, clear_value_count, true)) {
            return false;
        }
        if (subpass->has_depth_stencil_resolve_attachment) {
            uint32_t resolve_index = subpass->depth_stencil_resolve_attachment;
...
static bool append_graphics_begin_rendering_command(PdockerVkCommandBuffer *cmd) {
    uint32_t rendering_snapshot_index = UINT32_MAX;
...
static bool append_graphics_end_rendering_command(PdockerVkCommandBuffer *cmd) {
    PdockerVkGraphicsCommandRecord record;
    memset(&record, 0, sizeof(record));
    record.command_type = PDOCKER_GPU_GRAPHICS_V6_COMMAND_END_RENDERING;
    return append_graphics_command_record(cmd, &record);
...
static bool append_normalized_render_pass_begin(
    ...
    if (!append_render_pass_subpass_layout_transitions(cmd, 0, &rp->begin_dependency)) return false;
    if (!append_graphics_begin_rendering_command(cmd)) return false;
    cmd->dynamic_rendering_active = true;
    cmd->render_pass_active = false;
    cmd->active_subpass = 0;
...
static bool append_submit_sync_entry(
    ...
    if (!entries || !entry_count || *entry_count >= PDOCKER_GPU_VULKAN_GRAPHICS_V619_MAX_SUBMIT_SYNCS) {
        return false;
    }
    PdockerGpuVulkanGraphicsV619SubmitSyncEntry *entry = &entries[(*entry_count)++];
    memset(entry, 0, sizeof(*entry));
    entry->sync_type = sync_type;
    entry->stage_mask = stage_mask;
    if (sem) {
        entry->semaphore_id = sem->semaphore_id;
        entry->flags = sem->timeline
            ? PDOCKER_GPU_GRAPHICS_V619_SUBMIT_SYNC_TIMELINE
            : PDOCKER_GPU_GRAPHICS_V619_SUBMIT_SYNC_BINARY;
        entry->value = sem->timeline ? value : 0;
    }
    if (fence) entry->fence_id = fence->fence_id;
    return true;
}
...
static void set_submit2_metadata_override(const VkSubmitInfo2 *submit) {
    g_submit2_metadata_override = submit;
    g_submit2_metadata_command_index = PDOCKER_GPU_GRAPHICS_V621_COMMAND_BUFFER_INDEX_NONE;
}

static void set_submit2_metadata_command_index(uint32_t command_index) {
    g_submit2_metadata_command_index = command_index;
}

static void clear_submit2_metadata_override(void) {
    g_submit2_metadata_override = NULL;
    g_submit2_metadata_command_index = PDOCKER_GPU_GRAPHICS_V621_COMMAND_BUFFER_INDEX_NONE;
}
...
static bool command_buffer_needs_graphics_submit_sync_frame(const PdockerVkCommandBuffer *cmd) {
    ...
    bool has_executor_frame_command = false;
    for (uint32_t op_index = 0; op_index < cmd->command_op_count; ++op_index) {
        if (command_op_requires_graphics_executor_frame(cmd, op_index)) {
            has_executor_frame_command = true;
            break;
        }
    }
    if (!has_graphics_submit_frame && !has_executor_frame_command) {
        return false;
    }
    for (uint32_t i = 0; i < cmd->command_op_count; ++i) {
...
static bool graphics_mixed_submit_plan(
    ...
    for (uint32_t op_index = 0; op_index < cmd->command_op_count; ++op_index) {
        ...
        bool interleaved_between_draws = first_draw != UINT32_MAX &&
                                         ...
                                         command_op_is_graphics_interleavable_transfer_op(type);
        bool executor_frame_command = command_op_requires_graphics_executor_frame(cmd, op_index);
        if (command_op_is_graphics_interleavable_transfer_op(type)) {
            if (inside_gpu_frame && graphics_sequence_inside_active_rendering(cmd, op_index)) {
                if (reason_out) {
                    *reason_out = "graphics-mixed-transfer-inside-rendering-unimplemented";
                }
                return false;
            }
            if (inside_gpu_frame && !interleaved_between_draws && !executor_frame_command) {
                if (reason_out) {
        ...
        if (inside_gpu_frame && command_op_is_executor_compute_op(type)) {
            if (graphics_sequence_inside_active_rendering(cmd, op_index)) {
                if (reason_out) {
                    *reason_out = graphics_mixed_dispatch_inside_frame_reason(cmd, op_index);
                }
                return false;
            }
            continue;
...
static uint32_t count_graphics_sequence_segments_split_by_dispatch(
        const PdockerVkCommandBuffer *cmd,
        uint32_t first_gpu_op,
        uint32_t last_gpu_op) {
    ...
    if (last_gpu_op == UINT32_MAX) return 0;
    uint32_t segment_count = 0;
    uint32_t segment_begin = first_gpu_op;
    uint32_t sequence_end = last_gpu_op + 1u;
    for (uint32_t op_index = first_gpu_op;
         op_index < cmd->command_op_count && op_index < sequence_end;
         ++op_index) {
        const PdockerVkCommandOp *op = &cmd->command_ops[op_index];
        if (!command_op_is_executor_compute_op(op->type)) continue;
...
static int execute_graphics_mixed_gpu_sequence(
    ...
    uint32_t segment_total = count_graphics_sequence_segments_split_by_dispatch(
        cmd, first_gpu_op, last_gpu_op);
    if (segment_total == 0) return -EINVAL;
