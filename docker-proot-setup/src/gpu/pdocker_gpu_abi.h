...
#define PDOCKER_GPU_VULKAN_DISPATCH_V57_ABI_MINOR 7u
#define PDOCKER_GPU_VULKAN_DISPATCH_V58_ABI_MINOR 8u
#define PDOCKER_GPU_VULKAN_DISPATCH_V59_ABI_MINOR 9u
...
#define PDOCKER_GPU_VULKAN_DISPATCH_V5_ABI_MINOR_DESCRIPTOR_LAYOUT_FLAGS PDOCKER_GPU_VULKAN_DISPATCH_V58_ABI_MINOR
#define PDOCKER_GPU_VULKAN_DISPATCH_V5_ABI_MINOR_VARIABLE_DESCRIPTOR_COUNTS PDOCKER_GPU_VULKAN_DISPATCH_V59_ABI_MINOR
...
#define PDOCKER_GPU_VULKAN_DISPATCH_V57_HEADER_EXTENSION_SCHEMA_HASH 0x094aaf9b46be9136ull
#define PDOCKER_GPU_VULKAN_DISPATCH_V57_PUSH_CONSTANT_OP_SCHEMA_HASH 0x577f934a37175206ull
#define PDOCKER_GPU_VULKAN_DISPATCH_V58_HEADER_EXTENSION_SCHEMA_HASH 0x3931818679ff411cull
#define PDOCKER_GPU_VULKAN_DISPATCH_V58_DESCRIPTOR_SET_LAYOUT_FLAG_SCHEMA_HASH 0x6c385af6b82f1d85ull
#define PDOCKER_GPU_VULKAN_DISPATCH_V59_HEADER_EXTENSION_SCHEMA_HASH 0x3d656684f026e518ull
#define PDOCKER_GPU_VULKAN_DISPATCH_V59_VARIABLE_DESCRIPTOR_COUNT_SCHEMA_HASH 0x7808a01cc7e99aa7ull
...
/*
 * Legacy text dispatch (VULKAN_DISPATCH_V1..V4) still uses a compact command
 * string plus SCM_RIGHTS.  Keep its fd cap deliberately small so wide
 * buffer-only dispatches promote to the typed V5 frame instead of producing a
 * command string that can exceed the legacy text receiver.  Framed V5/V6 paths
 * use PDOCKER_GPU_TRANSPORT_MAX_PASSED_FDS directly.
 */
#define PDOCKER_GPU_VULKAN_TEXT_DISPATCH_MAX_PASSED_FDS 24u
#define PDOCKER_GPU_VULKAN_TEXT_DISPATCH_MAX_BINDINGS (PDOCKER_GPU_VULKAN_TEXT_DISPATCH_MAX_PASSED_FDS - 1u)
...
#define PDOCKER_GPU_VULKAN_DISPATCH_V59_MAX_VARIABLE_DESCRIPTOR_COUNTS PDOCKER_GPU_VULKAN_DISPATCH_V5_MAX_DESCRIPTORS
...
#define PDOCKER_GPU_VULKAN_DISPATCH_V57_MAX_PUSH_CONSTANT_OPS 4096u
...
#define PDOCKER_GPU_VULKAN_DISPATCH_V52_HEADER_EXTENSION_FIELDS(X) \
    X(image_layout_range_count, u32) \
    X(image_layout_range_entry_size, u32) \
    X(image_layout_range_table_offset, u64) \
    X(image_layout_range_table_size, u64) \
    X(image_layout_range_schema_hash, u64) \
    X(image_layout_range_table_hash, u64) \
    X(extension_hash, u64)
#define PDOCKER_GPU_VULKAN_DISPATCH_V52_HEADER_EXTENSION_FIELD_COUNT 7u

#define PDOCKER_GPU_VULKAN_DISPATCH_V52_IMAGE_LAYOUT_RANGE_FIELDS(X) \
    X(image_index, u32) \
    X(aspect_mask, u32) \
...
#define PDOCKER_GPU_VULKAN_DISPATCH_V52_IMAGE_LAYOUT_RANGE_FIELD_COUNT 9u

#define PDOCKER_GPU_VULKAN_DISPATCH_V53_HEADER_EXTENSION_FIELDS(X) \
    X(buffer_view_count, u32) \
    X(buffer_view_entry_size, u32) \
    X(buffer_view_table_offset, u64) \
    X(buffer_view_table_size, u64) \
    X(buffer_view_schema_hash, u64) \
    X(buffer_view_table_hash, u64) \
    X(extension_hash, u64)
#define PDOCKER_GPU_VULKAN_DISPATCH_V53_HEADER_EXTENSION_FIELD_COUNT 7u

#define PDOCKER_GPU_VULKAN_DISPATCH_V53_BUFFER_VIEW_FIELDS(X) \
    X(descriptor_index, u32) \
    X(resource_index, u32) \
    X(format, u32) \
    X(reserved0, u32) \
    X(buffer_view_id, u64) \
    X(buffer_offset, u64) \
    X(range, u64) \
    X(generation, u64)
...
#define PDOCKER_GPU_VULKAN_DISPATCH_V57_HEADER_EXTENSION_FIELDS(X) \
    X(push_constant_op_count, u32) \
    X(push_constant_op_entry_size, u32) \
    X(push_constant_op_table_offset, u64) \
    X(push_constant_op_table_size, u64) \
    X(push_constant_op_schema_hash, u64) \
    X(push_constant_op_table_hash, u64) \
    X(push_constant_data_offset, u64) \
    X(push_constant_data_size, u64) \
    X(push_constant_data_hash, u64) \
    X(extension_hash, u64)
#define PDOCKER_GPU_VULKAN_DISPATCH_V57_HEADER_EXTENSION_FIELD_COUNT 10u

#define PDOCKER_GPU_VULKAN_DISPATCH_V57_PUSH_CONSTANT_OP_FIELDS(X) \
    X(pipeline_layout_id, u64) \
    X(sequence, u32) \
    X(stage_flags, u32) \
    X(offset, u32) \
    ...
    X(data_hash, u64) \
    X(reserved0, u64)
#define PDOCKER_GPU_VULKAN_DISPATCH_V57_PUSH_CONSTANT_OP_FIELD_COUNT 8u

#define PDOCKER_GPU_VULKAN_DISPATCH_V58_HEADER_EXTENSION_FIELDS(X) \
    X(descriptor_set_layout_flag_count, u32) \
    X(descriptor_set_layout_flag_entry_size, u32) \
    X(descriptor_set_layout_flag_table_offset, u64) \
    X(descriptor_set_layout_flag_table_size, u64) \
    X(descriptor_set_layout_flag_schema_hash, u64) \
    X(descriptor_set_layout_flag_table_hash, u64) \
    X(extension_hash, u64)
#define PDOCKER_GPU_VULKAN_DISPATCH_V58_HEADER_EXTENSION_FIELD_COUNT 7u

#define PDOCKER_GPU_VULKAN_DISPATCH_V58_DESCRIPTOR_SET_LAYOUT_FLAG_FIELDS(X) \
    X(layout_id, u64) \
    X(create_flags, u64)
#define PDOCKER_GPU_VULKAN_DISPATCH_V58_DESCRIPTOR_SET_LAYOUT_FLAG_FIELD_COUNT 2u

#define PDOCKER_GPU_VULKAN_DISPATCH_V59_HEADER_EXTENSION_FIELDS(X) \
    X(variable_descriptor_count_count, u32) \
    X(variable_descriptor_count_entry_size, u32) \
    X(variable_descriptor_count_table_offset, u64) \
    X(variable_descriptor_count_table_size, u64) \
    X(variable_descriptor_count_schema_hash, u64) \
    X(variable_descriptor_count_table_hash, u64) \
    X(extension_hash, u64)
#define PDOCKER_GPU_VULKAN_DISPATCH_V59_HEADER_EXTENSION_FIELD_COUNT 7u

#define PDOCKER_GPU_VULKAN_DISPATCH_V59_VARIABLE_DESCRIPTOR_COUNT_FIELDS(X) \
    X(command_index, u32) \
    X(set_index, u32) \
    X(binding, u32) \
    X(actual_descriptor_count, u32) \
    X(layout_descriptor_count, u32) \
    X(reserved0, u32) \
    X(pipeline_layout_id, u64) \
    X(descriptor_set_layout_id, u64)
#define PDOCKER_GPU_VULKAN_DISPATCH_V59_VARIABLE_DESCRIPTOR_COUNT_FIELD_COUNT 8u
...
typedef struct PdockerGpuVulkanDispatchV52HeaderExtension {
    uint32_t image_layout_range_count;
    uint32_t image_layout_range_entry_size;
    uint64_t image_layout_range_table_offset;
    uint64_t image_layout_range_table_size;
    uint64_t image_layout_range_schema_hash;
    uint64_t image_layout_range_table_hash;
    uint64_t extension_hash;
} PdockerGpuVulkanDispatchV52HeaderExtension;

typedef struct PdockerGpuVulkanDispatchV52FrameHeader {
    PdockerGpuVulkanDispatchV5ObjectFrameHeader v51;
    PdockerGpuVulkanDispatchV52HeaderExtension v52;
} PdockerGpuVulkanDispatchV52FrameHeader;

typedef struct PdockerGpuVulkanDispatchV53HeaderExtension {
    uint32_t buffer_view_count;
    uint32_t buffer_view_entry_size;
    uint64_t buffer_view_table_offset;
    uint64_t buffer_view_table_size;
    uint64_t buffer_view_schema_hash;
    uint64_t buffer_view_table_hash;
    uint64_t extension_hash;
} PdockerGpuVulkanDispatchV53HeaderExtension;

typedef struct PdockerGpuVulkanDispatchV53FrameHeader {
    PdockerGpuVulkanDispatchV52FrameHeader v52;
...
typedef struct PdockerGpuVulkanDispatchV57HeaderExtension {
    uint32_t push_constant_op_count;
    uint32_t push_constant_op_entry_size;
    uint64_t push_constant_op_table_offset;
    uint64_t push_constant_op_table_size;
    uint64_t push_constant_op_schema_hash;
    uint64_t push_constant_op_table_hash;
    uint64_t push_constant_data_offset;
    uint64_t push_constant_data_size;
    uint64_t push_constant_data_hash;
    uint64_t extension_hash;
} PdockerGpuVulkanDispatchV57HeaderExtension;

typedef struct PdockerGpuVulkanDispatchV57FrameHeader {
    PdockerGpuVulkanDispatchV56FrameHeader v56;
    PdockerGpuVulkanDispatchV57HeaderExtension v57;
} PdockerGpuVulkanDispatchV57FrameHeader;

typedef struct PdockerGpuVulkanDispatchV58HeaderExtension {
    uint32_t descriptor_set_layout_flag_count;
    uint32_t descriptor_set_layout_flag_entry_size;
    uint64_t descriptor_set_layout_flag_table_offset;
    uint64_t descriptor_set_layout_flag_table_size;
    uint64_t descriptor_set_layout_flag_schema_hash;
    uint64_t descriptor_set_layout_flag_table_hash;
    uint64_t extension_hash;
} PdockerGpuVulkanDispatchV58HeaderExtension;

typedef struct PdockerGpuVulkanDispatchV58FrameHeader {
    PdockerGpuVulkanDispatchV57FrameHeader v57;
    PdockerGpuVulkanDispatchV58HeaderExtension v58;
} PdockerGpuVulkanDispatchV58FrameHeader;

typedef struct PdockerGpuVulkanDispatchV58DescriptorSetLayoutFlagEntry {
    uint64_t layout_id;
    uint64_t create_flags;
} PdockerGpuVulkanDispatchV58DescriptorSetLayoutFlagEntry;

typedef struct PdockerGpuVulkanDispatchV59HeaderExtension {
    uint32_t variable_descriptor_count_count;
    uint32_t variable_descriptor_count_entry_size;
    uint64_t variable_descriptor_count_table_offset;
    uint64_t variable_descriptor_count_table_size;
    uint64_t variable_descriptor_count_schema_hash;
    uint64_t variable_descriptor_count_table_hash;
    uint64_t extension_hash;
} PdockerGpuVulkanDispatchV59HeaderExtension;

typedef struct PdockerGpuVulkanDispatchV59FrameHeader {
    PdockerGpuVulkanDispatchV58FrameHeader v58;
    PdockerGpuVulkanDispatchV59HeaderExtension v59;
} PdockerGpuVulkanDispatchV59FrameHeader;

typedef struct PdockerGpuVulkanDispatchV59VariableDescriptorCountEntry {
    uint32_t command_index;
    uint32_t set_index;
    uint32_t binding;
    uint32_t actual_descriptor_count;
    uint32_t layout_descriptor_count;
    uint32_t reserved0;
    uint64_t pipeline_layout_id;
    uint64_t descriptor_set_layout_id;
} PdockerGpuVulkanDispatchV59VariableDescriptorCountEntry;
...
typedef struct PdockerGpuVulkanDispatchV57PushConstantOpEntry {
    uint64_t pipeline_layout_id;
    uint32_t sequence;
    uint32_t stage_flags;
    uint32_t offset;
 ...
 * clear-depth-stencil, image resolve/blit, attachment clear metadata,
 * submit sync metadata, and image layout range metadata.
...
#define PDOCKER_GPU_VULKAN_GRAPHICS_V622_ABI_MINOR 22u
#define PDOCKER_GPU_VULKAN_GRAPHICS_V623_ABI_MINOR 23u
#define PDOCKER_GPU_VULKAN_GRAPHICS_V624_ABI_MINOR 24u
#define PDOCKER_GPU_VULKAN_GRAPHICS_V625_ABI_MINOR 25u
#define PDOCKER_GPU_VULKAN_GRAPHICS_V626_ABI_MINOR 26u
...
#define PDOCKER_GPU_VULKAN_GRAPHICS_V619_HEADER_EXTENSION_SCHEMA_HASH 0xedf5f511a526f40cull
...
#define PDOCKER_GPU_VULKAN_GRAPHICS_V622_HEADER_EXTENSION_SCHEMA_HASH 0x0509e9f8f7c7376eull
#define PDOCKER_GPU_VULKAN_GRAPHICS_V622_MULTISAMPLE_STATE_SCHEMA_HASH 0xa6413f4bdcebaba5ull
#define PDOCKER_GPU_VULKAN_GRAPHICS_V623_HEADER_EXTENSION_SCHEMA_HASH 0x7a5edf9a4c38f190ull
#define PDOCKER_GPU_VULKAN_GRAPHICS_V623_TESSELLATION_STATE_SCHEMA_HASH 0x2f9149a1bdb70101ull
#define PDOCKER_GPU_VULKAN_GRAPHICS_V624_HEADER_EXTENSION_SCHEMA_HASH 0x36f40c37e14a8322ull
#define PDOCKER_GPU_VULKAN_GRAPHICS_V624_DESCRIPTOR_SET_LAYOUT_SCHEMA_HASH 0x68e5ecdcc4de709dull
#define PDOCKER_GPU_VULKAN_GRAPHICS_V624_PIPELINE_LAYOUT_SET_SCHEMA_HASH 0xf8ac8c54032c1be8ull
#define PDOCKER_GPU_VULKAN_GRAPHICS_V625_HEADER_EXTENSION_SCHEMA_HASH 0x371469776fdfdb0aull
#define PDOCKER_GPU_VULKAN_GRAPHICS_V625_DESCRIPTOR_BIND_SCHEMA_HASH 0x02c66ad395d542ffull
#define PDOCKER_GPU_VULKAN_GRAPHICS_V626_HEADER_EXTENSION_SCHEMA_HASH 0x42145786f757d854ull
#define PDOCKER_GPU_VULKAN_GRAPHICS_V626_EVENT_WAIT_REF_SCHEMA_HASH 0x7cb49d1e98ad96a8ull
#define PDOCKER_GPU_VULKAN_GRAPHICS_V627_HEADER_EXTENSION_SCHEMA_HASH 0x1f0b20113887d648ull
#define PDOCKER_GPU_VULKAN_GRAPHICS_V627_BUFFER_VIEW_SCHEMA_HASH 0x80faefc4342a79c8ull
#define PDOCKER_GPU_VULKAN_GRAPHICS_V628_HEADER_EXTENSION_SCHEMA_HASH 0x4f0ad63bf8b2d6e1ull
...
#define PDOCKER_GPU_VULKAN_GRAPHICS_V633_HEADER_EXTENSION_SCHEMA_HASH 0x106aa4dc5ef7d444ull
#define PDOCKER_GPU_VULKAN_GRAPHICS_V633_SUBPASS_VIEW_MASK_SCHEMA_HASH 0x1dc55923b60d3486ull
#define PDOCKER_GPU_VULKAN_GRAPHICS_V633_DEPTH_STENCIL_RESOLVE_SCHEMA_HASH 0xd8456ba96b41dc13ull
#define PDOCKER_GPU_VULKAN_GRAPHICS_V633_DEPENDENCY_VIEW_OFFSET_SCHEMA_HASH 0x79b1baf5e4c8d171ull
#define PDOCKER_GPU_VULKAN_GRAPHICS_V633_CORRELATION_MASK_SCHEMA_HASH 0xa4b93ab9d5bc875cull
#define PDOCKER_GPU_VULKAN_GRAPHICS_V634_HEADER_EXTENSION_SCHEMA_HASH 0x7e4276f568a15c9cull
#define PDOCKER_GPU_VULKAN_GRAPHICS_V634_FRAMEBUFFER_SCHEMA_HASH 0x9560ffe8864e487bull
...
#define PDOCKER_GPU_VULKAN_GRAPHICS_V622_MAX_MULTISAMPLE_STATES PDOCKER_GPU_VULKAN_GRAPHICS_V6_MAX_PIPELINES
#define PDOCKER_GPU_VULKAN_GRAPHICS_V622_MAX_SAMPLE_MASK_WORDS 1u
#define PDOCKER_GPU_VULKAN_GRAPHICS_V623_MAX_TESSELLATION_STATES PDOCKER_GPU_VULKAN_GRAPHICS_V6_MAX_PIPELINES
#define PDOCKER_GPU_VULKAN_GRAPHICS_V624_MAX_DESCRIPTOR_SET_LAYOUT_BINDINGS 8192u
#define PDOCKER_GPU_VULKAN_GRAPHICS_V624_MAX_PIPELINE_LAYOUT_SETS (PDOCKER_GPU_VULKAN_GRAPHICS_V6_MAX_PIPELINES * PDOCKER_GPU_MAX_VULKAN_DESCRIPTOR_SETS)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V625_MAX_DESCRIPTOR_BINDS PDOCKER_GPU_VULKAN_GRAPHICS_V6_MAX_COMMANDS
#define PDOCKER_GPU_VULKAN_GRAPHICS_V626_MAX_EVENT_WAIT_REFS PDOCKER_GPU_VULKAN_GRAPHICS_V6_MAX_COMMANDS
...
#define PDOCKER_GPU_VULKAN_GRAPHICS_V633_MAX_SUBPASS_VIEW_MASKS PDOCKER_GPU_VULKAN_GRAPHICS_V632_MAX_RENDER_PASS_SUBPASSES
#define PDOCKER_GPU_VULKAN_GRAPHICS_V633_MAX_DEPTH_STENCIL_RESOLVES PDOCKER_GPU_VULKAN_GRAPHICS_V632_MAX_RENDER_PASS_SUBPASSES
#define PDOCKER_GPU_VULKAN_GRAPHICS_V633_MAX_DEPENDENCY_VIEW_OFFSETS PDOCKER_GPU_VULKAN_GRAPHICS_V632_MAX_RENDER_PASS_DEPENDENCIES
#define PDOCKER_GPU_VULKAN_GRAPHICS_V633_MAX_CORRELATION_MASKS (PDOCKER_GPU_VULKAN_GRAPHICS_V632_MAX_RENDER_PASSES * PDOCKER_GPU_VULKAN_GRAPHICS_V6_MAX_ATTACHMENTS)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V634_MAX_FRAMEBUFFERS PDOCKER_GPU_VULKAN_GRAPHICS_V6_MAX_COMMANDS
...
#define PDOCKER_GPU_GRAPHICS_V65_STATIC_DEPTH_BIAS_FACTORS_PRESENT 0x00000020u
...
#define PDOCKER_GPU_VULKAN_GRAPHICS_V619_HEADER_EXTENSION_FIELDS(X) \
    X(submit_sync_count, u32) \
    X(submit_sync_entry_size, u32) \
    X(submit_sync_table_offset, u64) \
    X(submit_sync_table_size, u64) \
    X(submit_sync_schema_hash, u64) \
    X(submit_sync_table_hash, u64) \
    X(extension_hash, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V619_HEADER_EXTENSION_FIELD_COUNT 7u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V619_SUBMIT_SYNC_FIELDS(X) \
    X(sync_type, u32) \
    X(flags, u32) \
    X(semaphore_id, u64) \
    X(value, u64) \
    ...
    X(fence_id, u64) \
    X(reserved0, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V619_SUBMIT_SYNC_FIELD_COUNT 7u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V620_HEADER_EXTENSION_FIELDS(X) \
    X(image_layout_range_count, u32) \
    X(image_layout_range_entry_size, u32) \
    X(image_layout_range_table_offset, u64) \
    X(image_layout_range_table_size, u64) \
    ...
    X(image_layout_range_table_hash, u64) \
    X(extension_hash, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V620_HEADER_EXTENSION_FIELD_COUNT 7u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V620_IMAGE_LAYOUT_RANGE_FIELDS(X) \
    X(image_index, u32) \
    X(aspect_mask, u32) \
    X(base_mip_level, u32) \
    X(level_count, u32) \
...
#define PDOCKER_GPU_VULKAN_GRAPHICS_V622_HEADER_EXTENSION_FIELDS(X) \
    X(multisample_state_count, u32) \
    X(multisample_state_entry_size, u32) \
    X(multisample_state_table_offset, u64) \
    X(multisample_state_table_size, u64) \
    X(multisample_state_schema_hash, u64) \
    X(multisample_state_table_hash, u64) \
    X(extension_hash, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V622_HEADER_EXTENSION_FIELD_COUNT 7u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V622_MULTISAMPLE_STATE_FIELDS(X) \
    X(pipeline_index, u32) \
    X(flags, u32) \
    X(min_sample_shading_bits, u32) \
    X(sample_mask_word_count, u32) \
    X(sample_mask0, u32) \
    X(reserved0, u32) \
    X(reserved1, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V622_MULTISAMPLE_STATE_FIELD_COUNT 7u

#define PDOCKER_GPU_GRAPHICS_V622_MULTISAMPLE_SAMPLE_SHADING_ENABLE 0x00000001u
#define PDOCKER_GPU_GRAPHICS_V622_MULTISAMPLE_SAMPLE_MASK_PRESENT 0x00000002u
#define PDOCKER_GPU_GRAPHICS_V622_MULTISAMPLE_ALPHA_TO_COVERAGE_ENABLE 0x00000004u
#define PDOCKER_GPU_GRAPHICS_V622_MULTISAMPLE_ALPHA_TO_ONE_ENABLE 0x00000008u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V623_HEADER_EXTENSION_FIELDS(X) \
    X(tessellation_state_count, u32) \
    X(tessellation_state_entry_size, u32) \
    X(tessellation_state_table_offset, u64) \
    X(tessellation_state_table_size, u64) \
    X(tessellation_state_schema_hash, u64) \
    X(tessellation_state_table_hash, u64) \
    X(extension_hash, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V623_HEADER_EXTENSION_FIELD_COUNT 7u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V623_TESSELLATION_STATE_FIELDS(X) \
    X(pipeline_index, u32) \
    X(patch_control_points, u32) \
    X(flags, u32) \
    X(reserved0, u32) \
    X(reserved1, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V623_TESSELLATION_STATE_FIELD_COUNT 5u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V624_HEADER_EXTENSION_FIELDS(X) \
    X(descriptor_set_layout_count, u32) \
    X(descriptor_set_layout_entry_size, u32) \
    X(descriptor_set_layout_table_offset, u64) \
    X(descriptor_set_layout_table_size, u64) \
    ...
    X(pipeline_layout_table_hash, u64) \
    X(extension_hash, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V624_HEADER_EXTENSION_FIELD_COUNT 13u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V624_DESCRIPTOR_SET_LAYOUT_FIELDS(X) \
    X(layout_id, u64) \
    X(binding, u32) \
    X(descriptor_type, u32) \
    X(descriptor_count, u32) \
    ...
    X(immutable_sampler_count, u32) \
    X(reserved0, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V624_DESCRIPTOR_SET_LAYOUT_FIELD_COUNT 8u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V624_PIPELINE_LAYOUT_SET_FIELDS(X) \
    X(pipeline_layout_id, u64) \
    X(set_index, u32) \
    X(reserved0, u32) \
    X(descriptor_set_layout_id, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V624_PIPELINE_LAYOUT_SET_FIELD_COUNT 4u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V625_HEADER_EXTENSION_FIELDS(X) \
    X(descriptor_bind_count, u32) \
    X(descriptor_bind_entry_size, u32) \
    X(descriptor_bind_table_offset, u64) \
    X(descriptor_bind_table_size, u64) \
    X(descriptor_bind_schema_hash, u64) \
    X(descriptor_bind_table_hash, u64) \
    X(extension_hash, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V625_HEADER_EXTENSION_FIELD_COUNT 7u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V625_DESCRIPTOR_BIND_FIELDS(X) \
    X(command_index, u32) \
    X(first_set, u32) \
    X(descriptor_set_count, u32) \
    X(first_dynamic_offset, u32) \
    X(dynamic_offset_count, u32) \
    X(reserved0, u32) \
    X(pipeline_layout_id, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V625_DESCRIPTOR_BIND_FIELD_COUNT 7u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V626_HEADER_EXTENSION_FIELDS(X) \
    X(event_wait_ref_count, u32) \
    X(event_wait_ref_entry_size, u32) \
    X(event_wait_ref_table_offset, u64) \
    X(event_wait_ref_table_size, u64) \
    X(event_wait_ref_schema_hash, u64) \
    X(event_wait_ref_table_hash, u64) \
    X(extension_hash, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V626_HEADER_EXTENSION_FIELD_COUNT 7u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V626_EVENT_WAIT_REF_FIELDS(X) \
    X(command_index, u32) \
    X(event_ordinal, u32) \
    X(event_id, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V626_EVENT_WAIT_REF_FIELD_COUNT 3u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V627_HEADER_EXTENSION_FIELDS(X) \
    X(buffer_view_count, u32) \
    X(buffer_view_entry_size, u32) \
    X(buffer_view_table_offset, u64) \
    X(buffer_view_table_size, u64) \
    ...
    X(buffer_view_table_hash, u64) \
    X(extension_hash, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V627_HEADER_EXTENSION_FIELD_COUNT 7u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V627_BUFFER_VIEW_FIELDS(X) \
    X(command_index, u32) \
    X(descriptor_index, u32) \
    X(resource_index, u32) \
    X(format, u32) \
    ...
    X(range, u64) \
    X(generation, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V627_BUFFER_VIEW_FIELD_COUNT 8u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V628_HEADER_EXTENSION_FIELDS(X) \
    X(push_constant_range_count, u32) \
    X(push_constant_range_entry_size, u32) \
    X(push_constant_range_table_offset, u64) \
    X(push_constant_range_table_size, u64) \
    ...
    X(push_constant_range_table_hash, u64) \
    X(extension_hash, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V628_HEADER_EXTENSION_FIELD_COUNT 7u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V628_PUSH_CONSTANT_RANGE_FIELDS(X) \
    X(pipeline_layout_id, u64) \
    X(range_index, u32) \
    X(stage_flags, u32) \
    X(offset, u32) \
    X(size, u32)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V628_PUSH_CONSTANT_RANGE_FIELD_COUNT 5u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V629_HEADER_EXTENSION_FIELDS(X) \
    X(variable_descriptor_count_count, u32) \
    X(variable_descriptor_count_entry_size, u32) \
    X(variable_descriptor_count_table_offset, u64) \
    X(variable_descriptor_count_table_size, u64) \
...
#define PDOCKER_GPU_VULKAN_GRAPHICS_V629_VARIABLE_DESCRIPTOR_COUNT_FIELDS(X) \
    ...
    X(pipeline_layout_id, u64) \
    X(descriptor_set_layout_id, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V629_VARIABLE_DESCRIPTOR_COUNT_FIELD_COUNT 8u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V631_HEADER_EXTENSION_FIELDS(X) \
    X(descriptor_set_layout_flag_count, u32) \
    X(descriptor_set_layout_flag_entry_size, u32) \
    X(descriptor_set_layout_flag_table_offset, u64) \
    X(descriptor_set_layout_flag_table_size, u64) \
    ...
    X(descriptor_set_layout_flag_table_hash, u64) \
    X(extension_hash, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V631_HEADER_EXTENSION_FIELD_COUNT 7u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V631_DESCRIPTOR_SET_LAYOUT_FLAG_FIELDS(X) \
    X(layout_id, u64) \
    X(create_flags, u64)
...
#define PDOCKER_GPU_VULKAN_GRAPHICS_V633_HEADER_EXTENSION_FIELDS(X) \
    X(subpass_view_mask_count, u32) \
    X(subpass_view_mask_entry_size, u32) \
    X(subpass_view_mask_table_offset, u64) \
    X(subpass_view_mask_table_size, u64) \
    X(subpass_view_mask_schema_hash, u64) \
    X(subpass_view_mask_table_hash, u64) \
    X(depth_stencil_resolve_count, u32) \
    X(depth_stencil_resolve_entry_size, u32) \
    X(depth_stencil_resolve_table_offset, u64) \
    X(depth_stencil_resolve_table_size, u64) \
    X(depth_stencil_resolve_schema_hash, u64) \
    X(depth_stencil_resolve_table_hash, u64) \
    X(dependency_view_offset_count, u32) \
    X(dependency_view_offset_entry_size, u32) \
    X(dependency_view_offset_table_offset, u64) \
    X(dependency_view_offset_table_size, u64) \
    X(dependency_view_offset_schema_hash, u64) \
    X(dependency_view_offset_table_hash, u64) \
    X(correlation_mask_count, u32) \
    X(correlation_mask_entry_size, u32) \
    X(correlation_mask_table_offset, u64) \
    X(correlation_mask_table_size, u64) \
    X(correlation_mask_schema_hash, u64) \
    X(correlation_mask_table_hash, u64) \
    X(extension_hash, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V633_HEADER_EXTENSION_FIELD_COUNT 25u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V633_SUBPASS_VIEW_MASK_FIELDS(X) \
    X(render_pass_id, u64) \
    X(subpass_index, u32) \
    X(view_mask, u32)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V633_SUBPASS_VIEW_MASK_FIELD_COUNT 3u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V633_DEPTH_STENCIL_RESOLVE_FIELDS(X) \
    X(render_pass_id, u64) \
    X(subpass_index, u32) \
    X(depth_resolve_mode, u32) \
    X(stencil_resolve_mode, u32) \
    X(attachment_index, u32) \
    X(layout, u32) \
    X(aspect_mask, u32) \
    X(flags, u32) \
    X(reserved0, u32)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V633_DEPTH_STENCIL_RESOLVE_FIELD_COUNT 9u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V633_DEPENDENCY_VIEW_OFFSET_FIELDS(X) \
    X(render_pass_id, u64) \
    X(dependency_index, u32) \
    X(view_offset_bits, u32) \
    X(flags, u32) \
    X(reserved0, u32)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V633_DEPENDENCY_VIEW_OFFSET_FIELD_COUNT 5u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V633_CORRELATION_MASK_FIELDS(X) \
    X(render_pass_id, u64) \
    X(mask_index, u32) \
    X(correlation_mask, u32)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V633_CORRELATION_MASK_FIELD_COUNT 3u

#define PDOCKER_GPU_GRAPHICS_V633_DEPTH_STENCIL_RESOLVE_ATTACHMENT_PRESENT 0x00000001u
#define PDOCKER_GPU_GRAPHICS_V633_DEPENDENCY_VIEW_OFFSET_PRESENT 0x00000001u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V634_HEADER_EXTENSION_FIELDS(X) \
    X(framebuffer_count, u32) \
    X(framebuffer_entry_size, u32) \
    X(framebuffer_table_offset, u64) \
    X(framebuffer_table_size, u64) \
    X(framebuffer_schema_hash, u64) \
    X(framebuffer_table_hash, u64) \
    X(framebuffer_attachment_count, u32) \
    X(framebuffer_attachment_entry_size, u32) \
    X(framebuffer_attachment_table_offset, u64) \
    X(framebuffer_attachment_table_size, u64) \
    X(framebuffer_attachment_schema_hash, u64) \
    X(framebuffer_attachment_table_hash, u64) \
    X(render_pass_command_count, u32) \
    X(render_pass_command_entry_size, u32) \
    X(render_pass_command_table_offset, u64) \
    X(render_pass_command_table_size, u64) \
    X(render_pass_command_schema_hash, u64) \
    X(render_pass_command_table_hash, u64) \
    X(render_pass_clear_value_count, u32) \
    X(render_pass_clear_value_entry_size, u32) \
    X(render_pass_clear_value_table_offset, u64) \
    X(render_pass_clear_value_table_size, u64) \
    X(render_pass_clear_value_schema_hash, u64) \
    X(render_pass_clear_value_table_hash, u64) \
    X(extension_hash, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V634_HEADER_EXTENSION_FIELD_COUNT 25u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V634_FRAMEBUFFER_FIELDS(X) \
    X(framebuffer_id, u64) \
    X(render_pass_id, u64) \
    X(attachment_first, u32) \
    X(attachment_count, u32) \
    X(width, u32) \
    ...
    X(generation, u64) \
    X(reserved0, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V634_FRAMEBUFFER_FIELD_COUNT 10u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V634_FRAMEBUFFER_ATTACHMENT_FIELDS(X) \
    X(framebuffer_id, u64) \
    X(attachment_index, u32) \
    X(image_view_index, u32) \
    X(image_view_id, u64) \
    X(image_view_generation, u64) \
    X(reserved0, u64)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V634_FRAMEBUFFER_ATTACHMENT_FIELD_COUNT 6u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V634_RENDER_PASS_COMMAND_FIELDS(X) \
    X(command_index, u32) \
    X(op, u32) \
    X(contents, u32) \
    X(flags, u32) \
    ...
    X(framebuffer_id, u64) \
    X(subpass_index, u32) \
    X(clear_value_first, u32) \
    X(clear_value_count, u32) \
    X(render_area_offset_x, i32) \
    X(render_area_offset_y, i32) \
    X(render_area_extent_width, u32) \
    X(render_area_extent_height, u32) \
    X(reserved0, u32)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V634_RENDER_PASS_COMMAND_FIELD_COUNT 14u

#define PDOCKER_GPU_VULKAN_GRAPHICS_V634_RENDER_PASS_CLEAR_VALUE_FIELDS(X) \
    X(command_index, u32) \
    X(attachment_index, u32) \
    X(value0, u32) \
    X(value1, u32) \
    X(value2, u32) \
    X(value3, u32)
#define PDOCKER_GPU_VULKAN_GRAPHICS_V634_RENDER_PASS_CLEAR_VALUE_FIELD_COUNT 6u
...
#define PDOCKER_GPU_GRAPHICS_V6_COMMAND_SET_EVENT 28u
#define PDOCKER_GPU_GRAPHICS_V6_COMMAND_RESET_EVENT 29u
#define PDOCKER_GPU_GRAPHICS_V6_COMMAND_WAIT_EVENT 30u
#define PDOCKER_GPU_GRAPHICS_V6_COMMAND_VERTEX_STRIDES_PRESENT 0x00000001u
...
#define PDOCKER_GPU_GRAPHICS_V617_QUERY_OP_END 4u
...
#define PDOCKER_GPU_GRAPHICS_V619_SUBMIT_SYNC_WAIT 1u
#define PDOCKER_GPU_GRAPHICS_V619_SUBMIT_SYNC_SIGNAL 2u
#define PDOCKER_GPU_GRAPHICS_V619_SUBMIT_SYNC_FENCE 3u
#define PDOCKER_GPU_GRAPHICS_V619_SUBMIT_SYNC_TIMELINE 0x00000001u
#define PDOCKER_GPU_GRAPHICS_V619_SUBMIT_SYNC_BINARY 0x00000002u
...
typedef struct PdockerGpuVulkanGraphicsV619HeaderExtension {
    uint32_t submit_sync_count;
    uint32_t submit_sync_entry_size;
    uint64_t submit_sync_table_offset;
    uint64_t submit_sync_table_size;
    uint64_t submit_sync_schema_hash;
    uint64_t submit_sync_table_hash;
    uint64_t extension_hash;
} PdockerGpuVulkanGraphicsV619HeaderExtension;

typedef struct PdockerGpuVulkanGraphicsV619FrameHeader {
    PdockerGpuVulkanGraphicsV6FrameHeader base;
    PdockerGpuVulkanGraphicsV61HeaderExtension v61;
    PdockerGpuVulkanGraphicsV62HeaderExtension v62;
    PdockerGpuVulkanGraphicsV63HeaderExtension v63;
...
} PdockerGpuVulkanGraphicsV619FrameHeader;

typedef struct PdockerGpuVulkanGraphicsV620HeaderExtension {
    uint32_t image_layout_range_count;
    uint32_t image_layout_range_entry_size;
    uint64_t image_layout_range_table_offset;
    uint64_t image_layout_range_table_size;
    ...
    uint64_t image_layout_range_table_hash;
    uint64_t extension_hash;
} PdockerGpuVulkanGraphicsV620HeaderExtension;

typedef struct PdockerGpuVulkanGraphicsV620FrameHeader {
    PdockerGpuVulkanGraphicsV6FrameHeader base;
    PdockerGpuVulkanGraphicsV61HeaderExtension v61;
    PdockerGpuVulkanGraphicsV62HeaderExtension v62;
    PdockerGpuVulkanGraphicsV63HeaderExtension v63;
...
typedef struct PdockerGpuVulkanGraphicsV622HeaderExtension {
    uint32_t multisample_state_count;
    uint32_t multisample_state_entry_size;
    uint64_t multisample_state_table_offset;
    uint64_t multisample_state_table_size;
    uint64_t multisample_state_schema_hash;
    uint64_t multisample_state_table_hash;
    uint64_t extension_hash;
} PdockerGpuVulkanGraphicsV622HeaderExtension;

typedef struct PdockerGpuVulkanGraphicsV623HeaderExtension {
    uint32_t tessellation_state_count;
    uint32_t tessellation_state_entry_size;
    uint64_t tessellation_state_table_offset;
    uint64_t tessellation_state_table_size;
    uint64_t tessellation_state_schema_hash;
    uint64_t tessellation_state_table_hash;
    uint64_t extension_hash;
} PdockerGpuVulkanGraphicsV623HeaderExtension;

typedef struct PdockerGpuVulkanGraphicsV624HeaderExtension {
    uint32_t descriptor_set_layout_count;
    uint32_t descriptor_set_layout_entry_size;
    uint64_t descriptor_set_layout_table_offset;
    uint64_t descriptor_set_layout_table_size;
...
typedef struct PdockerGpuVulkanGraphicsV622FrameHeader {
    PdockerGpuVulkanGraphicsV6FrameHeader base;
    PdockerGpuVulkanGraphicsV61HeaderExtension v61;
    PdockerGpuVulkanGraphicsV62HeaderExtension v62;
    PdockerGpuVulkanGraphicsV63HeaderExtension v63;
    ...
    PdockerGpuVulkanGraphicsV620HeaderExtension v620;
    PdockerGpuVulkanGraphicsV621HeaderExtension v621;
    PdockerGpuVulkanGraphicsV622HeaderExtension v622;
} PdockerGpuVulkanGraphicsV622FrameHeader;

typedef struct PdockerGpuVulkanGraphicsV623FrameHeader {
    PdockerGpuVulkanGraphicsV6FrameHeader base;
    PdockerGpuVulkanGraphicsV61HeaderExtension v61;
    PdockerGpuVulkanGraphicsV62HeaderExtension v62;
    PdockerGpuVulkanGraphicsV63HeaderExtension v63;
    ...
    PdockerGpuVulkanGraphicsV621HeaderExtension v621;
    PdockerGpuVulkanGraphicsV622HeaderExtension v622;
    PdockerGpuVulkanGraphicsV623HeaderExtension v623;
} PdockerGpuVulkanGraphicsV623FrameHeader;

typedef struct PdockerGpuVulkanGraphicsV625HeaderExtension {
    uint32_t descriptor_bind_count;
    uint32_t descriptor_bind_entry_size;
    uint64_t descriptor_bind_table_offset;
    uint64_t descriptor_bind_table_size;
    uint64_t descriptor_bind_schema_hash;
    uint64_t descriptor_bind_table_hash;
    uint64_t extension_hash;
} PdockerGpuVulkanGraphicsV625HeaderExtension;

typedef struct PdockerGpuVulkanGraphicsV626HeaderExtension {
    uint32_t event_wait_ref_count;
    uint32_t event_wait_ref_entry_size;
    uint64_t event_wait_ref_table_offset;
    uint64_t event_wait_ref_table_size;
    uint64_t event_wait_ref_schema_hash;
    uint64_t event_wait_ref_table_hash;
    uint64_t extension_hash;
} PdockerGpuVulkanGraphicsV626HeaderExtension;

typedef struct PdockerGpuVulkanGraphicsV627HeaderExtension {
    uint32_t buffer_view_count;
    uint32_t buffer_view_entry_size;
    uint64_t buffer_view_table_offset;
    uint64_t buffer_view_table_size;
    ...
    uint64_t buffer_view_table_hash;
    uint64_t extension_hash;
} PdockerGpuVulkanGraphicsV627HeaderExtension;

typedef struct PdockerGpuVulkanGraphicsV628HeaderExtension {
    uint32_t push_constant_range_count;
    uint32_t push_constant_range_entry_size;
    uint64_t push_constant_range_table_offset;
    uint64_t push_constant_range_table_size;
    ...
    uint64_t push_constant_range_table_hash;
    uint64_t extension_hash;
} PdockerGpuVulkanGraphicsV628HeaderExtension;

typedef struct PdockerGpuVulkanGraphicsV629HeaderExtension {
    uint32_t variable_descriptor_count_count;
    uint32_t variable_descriptor_count_entry_size;
    uint64_t variable_descriptor_count_table_offset;
    uint64_t variable_descriptor_count_table_size;
...
typedef struct PdockerGpuVulkanGraphicsV624FrameHeader {
    ...
    PdockerGpuVulkanGraphicsV622HeaderExtension v622;
    PdockerGpuVulkanGraphicsV623HeaderExtension v623;
    PdockerGpuVulkanGraphicsV624HeaderExtension v624;
} PdockerGpuVulkanGraphicsV624FrameHeader;

typedef struct PdockerGpuVulkanGraphicsV625FrameHeader {
    PdockerGpuVulkanGraphicsV6FrameHeader base;
    PdockerGpuVulkanGraphicsV61HeaderExtension v61;
    PdockerGpuVulkanGraphicsV62HeaderExtension v62;
    PdockerGpuVulkanGraphicsV63HeaderExtension v63;
    ...
    PdockerGpuVulkanGraphicsV623HeaderExtension v623;
    PdockerGpuVulkanGraphicsV624HeaderExtension v624;
    PdockerGpuVulkanGraphicsV625HeaderExtension v625;
} PdockerGpuVulkanGraphicsV625FrameHeader;

typedef struct PdockerGpuVulkanGraphicsV626FrameHeader {
    PdockerGpuVulkanGraphicsV6FrameHeader base;
    PdockerGpuVulkanGraphicsV61HeaderExtension v61;
    PdockerGpuVulkanGraphicsV62HeaderExtension v62;
    PdockerGpuVulkanGraphicsV63HeaderExtension v63;
    ...
    PdockerGpuVulkanGraphicsV624HeaderExtension v624;
    PdockerGpuVulkanGraphicsV625HeaderExtension v625;
    PdockerGpuVulkanGraphicsV626HeaderExtension v626;
} PdockerGpuVulkanGraphicsV626FrameHeader;

typedef struct PdockerGpuVulkanGraphicsV627FrameHeader {
    PdockerGpuVulkanGraphicsV6FrameHeader base;
    PdockerGpuVulkanGraphicsV61HeaderExtension v61;
    PdockerGpuVulkanGraphicsV62HeaderExtension v62;
    PdockerGpuVulkanGraphicsV63HeaderExtension v63;
    ...
    PdockerGpuVulkanGraphicsV625HeaderExtension v625;
    PdockerGpuVulkanGraphicsV626HeaderExtension v626;
    PdockerGpuVulkanGraphicsV627HeaderExtension v627;
} PdockerGpuVulkanGraphicsV627FrameHeader;

typedef struct PdockerGpuVulkanGraphicsV628FrameHeader {
    PdockerGpuVulkanGraphicsV6FrameHeader base;
    PdockerGpuVulkanGraphicsV61HeaderExtension v61;
    PdockerGpuVulkanGraphicsV62HeaderExtension v62;
    PdockerGpuVulkanGraphicsV63HeaderExtension v63;
...
} PdockerGpuVulkanGraphicsV628FrameHeader;

typedef struct PdockerGpuVulkanGraphicsV629FrameHeader {
    PdockerGpuVulkanGraphicsV6FrameHeader base;
    PdockerGpuVulkanGraphicsV61HeaderExtension v61;
    PdockerGpuVulkanGraphicsV62HeaderExtension v62;
    PdockerGpuVulkanGraphicsV63HeaderExtension v63;
...
typedef struct PdockerGpuVulkanGraphicsV619SubmitSyncEntry {
    uint32_t sync_type;
    uint32_t flags;
...
typedef struct PdockerGpuVulkanGraphicsV620ImageLayoutRangeEntry {
    uint32_t image_index;
    uint32_t aspect_mask;
    uint32_t base_mip_level;
    uint32_t level_count;
...
typedef struct PdockerGpuVulkanGraphicsV622MultisampleStateEntry {
    uint32_t pipeline_index;
    uint32_t flags;
    uint32_t min_sample_shading_bits;
    uint32_t sample_mask_word_count;
    uint32_t sample_mask0;
    uint32_t reserved0;
    uint64_t reserved1;
} PdockerGpuVulkanGraphicsV622MultisampleStateEntry;

typedef struct PdockerGpuVulkanGraphicsV623TessellationStateEntry {
    uint32_t pipeline_index;
...
typedef struct PdockerGpuVulkanGraphicsV624DescriptorSetLayoutEntry {
    uint64_t layout_id;
    uint32_t binding;
    uint32_t descriptor_type;
    uint32_t descriptor_count;
...
typedef struct PdockerGpuVulkanGraphicsV624PipelineLayoutSetEntry {
    uint64_t pipeline_layout_id;
    uint32_t set_index;
    uint32_t reserved0;
    uint64_t descriptor_set_layout_id;
...
typedef struct PdockerGpuVulkanGraphicsV625DescriptorBindEntry {
    uint32_t command_index;
    uint32_t first_set;
...
typedef struct PdockerGpuVulkanGraphicsV626EventWaitRefEntry {
    uint32_t command_index;
    uint32_t event_ordinal;
    uint64_t event_id;
} PdockerGpuVulkanGraphicsV626EventWaitRefEntry;

typedef struct PdockerGpuVulkanGraphicsV627BufferViewEntry {
    uint32_t command_index;
    uint32_t descriptor_index;
    uint32_t resource_index;
    uint32_t format;
    ...
    uint64_t range;
    uint64_t generation;
} PdockerGpuVulkanGraphicsV627BufferViewEntry;

typedef struct PdockerGpuVulkanGraphicsV628PushConstantRangeEntry {
    uint64_t pipeline_layout_id;
    uint32_t range_index;
    uint32_t stage_flags;
    uint32_t offset;
...
typedef struct PdockerGpuVulkanGraphicsV629VariableDescriptorCountEntry {
    uint32_t command_index;
    uint32_t set_index;
    uint32_t binding;
    uint32_t actual_descriptor_count;
...
typedef struct PdockerGpuVulkanGraphicsV631HeaderExtension {
    uint32_t descriptor_set_layout_flag_count;
    uint32_t descriptor_set_layout_flag_entry_size;
    uint64_t descriptor_set_layout_flag_table_offset;
    uint64_t descriptor_set_layout_flag_table_size;
    ...
    uint64_t descriptor_set_layout_flag_table_hash;
    uint64_t extension_hash;
} PdockerGpuVulkanGraphicsV631HeaderExtension;

typedef struct PdockerGpuVulkanGraphicsV631FrameHeader {
    PdockerGpuVulkanGraphicsV630FrameHeader v630;
    PdockerGpuVulkanGraphicsV631HeaderExtension v631;
} PdockerGpuVulkanGraphicsV631FrameHeader;

typedef struct PdockerGpuVulkanGraphicsV631DescriptorSetLayoutFlagEntry {
    uint64_t layout_id;
    uint64_t create_flags;
...
typedef struct PdockerGpuVulkanGraphicsV633HeaderExtension {
    uint32_t subpass_view_mask_count;
    uint32_t subpass_view_mask_entry_size;
    uint64_t subpass_view_mask_table_offset;
    uint64_t subpass_view_mask_table_size;
    uint64_t subpass_view_mask_schema_hash;
    uint64_t subpass_view_mask_table_hash;
    uint32_t depth_stencil_resolve_count;
    uint32_t depth_stencil_resolve_entry_size;
    uint64_t depth_stencil_resolve_table_offset;
    uint64_t depth_stencil_resolve_table_size;
    uint64_t depth_stencil_resolve_schema_hash;
    uint64_t depth_stencil_resolve_table_hash;
    uint32_t dependency_view_offset_count;
    uint32_t dependency_view_offset_entry_size;
    uint64_t dependency_view_offset_table_offset;
    uint64_t dependency_view_offset_table_size;
    uint64_t dependency_view_offset_schema_hash;
    uint64_t dependency_view_offset_table_hash;
    uint32_t correlation_mask_count;
    uint32_t correlation_mask_entry_size;
    uint64_t correlation_mask_table_offset;
    uint64_t correlation_mask_table_size;
    uint64_t correlation_mask_schema_hash;
    uint64_t correlation_mask_table_hash;
    uint64_t extension_hash;
} PdockerGpuVulkanGraphicsV633HeaderExtension;

typedef struct PdockerGpuVulkanGraphicsV633FrameHeader {
    PdockerGpuVulkanGraphicsV632FrameHeader v632;
    PdockerGpuVulkanGraphicsV633HeaderExtension v633;
} PdockerGpuVulkanGraphicsV633FrameHeader;

typedef struct PdockerGpuVulkanGraphicsV633SubpassViewMaskEntry {
    uint64_t render_pass_id;
    uint32_t subpass_index;
    uint32_t view_mask;
} PdockerGpuVulkanGraphicsV633SubpassViewMaskEntry;

typedef struct PdockerGpuVulkanGraphicsV633DepthStencilResolveEntry {
    uint64_t render_pass_id;
    uint32_t subpass_index;
    uint32_t depth_resolve_mode;
    uint32_t stencil_resolve_mode;
    uint32_t attachment_index;
    uint32_t layout;
    uint32_t aspect_mask;
    uint32_t flags;
...
typedef struct PdockerGpuVulkanGraphicsV633DependencyViewOffsetEntry {
    uint64_t render_pass_id;
    uint32_t dependency_index;
    uint32_t view_offset_bits;
    uint32_t flags;
    uint32_t reserved0;
} PdockerGpuVulkanGraphicsV633DependencyViewOffsetEntry;

typedef struct PdockerGpuVulkanGraphicsV633CorrelationMaskEntry {
    uint64_t render_pass_id;
    uint32_t mask_index;
    uint32_t correlation_mask;
} PdockerGpuVulkanGraphicsV633CorrelationMaskEntry;

typedef struct PdockerGpuVulkanGraphicsV634HeaderExtension {
    uint32_t framebuffer_count;
    uint32_t framebuffer_entry_size;
    uint64_t framebuffer_table_offset;
    uint64_t framebuffer_table_size;
    uint64_t framebuffer_schema_hash;
    uint64_t framebuffer_table_hash;
    uint32_t framebuffer_attachment_count;
    uint32_t framebuffer_attachment_entry_size;
    uint64_t framebuffer_attachment_table_offset;
    uint64_t framebuffer_attachment_table_size;
    uint64_t framebuffer_attachment_schema_hash;
    uint64_t framebuffer_attachment_table_hash;
    uint32_t render_pass_command_count;
    uint32_t render_pass_command_entry_size;
    uint64_t render_pass_command_table_offset;
    uint64_t render_pass_command_table_size;
    uint64_t render_pass_command_schema_hash;
    uint64_t render_pass_command_table_hash;
    uint32_t render_pass_clear_value_count;
    uint32_t render_pass_clear_value_entry_size;
    uint64_t render_pass_clear_value_table_offset;
    uint64_t render_pass_clear_value_table_size;
    uint64_t render_pass_clear_value_schema_hash;
    uint64_t render_pass_clear_value_table_hash;
    uint64_t extension_hash;
} PdockerGpuVulkanGraphicsV634HeaderExtension;

typedef struct PdockerGpuVulkanGraphicsV634FrameHeader {
    PdockerGpuVulkanGraphicsV633FrameHeader v633;
    PdockerGpuVulkanGraphicsV634HeaderExtension v634;
} PdockerGpuVulkanGraphicsV634FrameHeader;

typedef struct PdockerGpuVulkanGraphicsV634FramebufferEntry {
...
typedef struct PdockerGpuVulkanGraphicsV634FramebufferAttachmentEntry {
    uint64_t framebuffer_id;
    uint32_t attachment_index;
    uint32_t image_view_index;
    uint64_t image_view_id;
    uint64_t image_view_generation;
    uint64_t reserved0;
} PdockerGpuVulkanGraphicsV634FramebufferAttachmentEntry;

typedef struct PdockerGpuVulkanGraphicsV634RenderPassCommandEntry {
    uint32_t command_index;
    uint32_t op;
    uint32_t contents;
    uint32_t flags;
...
typedef struct PdockerGpuVulkanGraphicsV634RenderPassClearValueEntry {
    uint32_t command_index;
    uint32_t attachment_index;
    uint32_t value0;
    uint32_t value1;
