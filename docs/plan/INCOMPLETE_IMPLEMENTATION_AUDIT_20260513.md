...
| llama GPU Q6_K and env propagation | `docs/plan/TODO.md`, `docs/plan/LLAMA_GPU_BRIDGE_NEXT_STEPS.md`, `docs/test/LLAMA_GPU_CORRECTNESS_20260507.md` | NGL=1 still fails required probes; Q6_K-like hash `0x274f68a67dfef210` remains the main blocker. `d5ce2e8` made pdockerd runtime defaults manifest-driven and host-guarded against UI/compose/compare/verifier drift, but device artifacts still must prove config propagation and Q6_K correctness together. | GPU acceleration cannot be claimed; prompts can return wrong tokens; a result can look path-dependent rather than reproducible. | Keep llama.cpp unchanged; add/verify Q6_K oracle or pass-through evidence, descriptor/local-size evidence, real-device config-propagation evidence from the synchronized GPU env manifest, and correctness gate before any performance claim. |
...
| COW/overlay mutation safety | `docs/design/COW_OVERLAY_STORAGE.md`, `docs/plan/TODO.md` | Host-local `libcow` and copy-up/metadata verifier coverage now exists for the major mutation classes, but Android daemon/helper kill-at-step evidence is still not promoted. | Layer/upperdir metadata can become inconsistent after crash/OOM on a real device if recovery assumptions diverge from host fixtures. | Add adb/run-as kill-at-step evidence for copy-up, rename, whiteout, archive, and metadata checkpoints before closing the device gate. |
...
| Area | Evidence | Current incomplete state | User impact | Next action |
| --- | --- | --- | --- | --- |
| Active port mapping | `pdockerd`, `TODO.md`, `tests/feature_scenarios.json` | The active/inactive/planned/conflict truth slice is now evidence-based and no longer pure metadata, but real forwarding/proxy or syscall-rewrite proof is still open. | `-p`/Compose ports can report truthful inactive/conflict states, but Docker-like reachability remains limited until forwarding exists. | Keep inactive/conflict wording; implement localhost proxy or syscall rewrite plus forwarding/conflict tests. |
