...
| --- | --- | --- | --- | --- |
| Llama GPU Q6 classifier/oracle boundary | Main agent | GPU bridge code plus llama GPU compare/verifier artifacts | Strict safe-kernel diagnostic enabled/classified in `5dc330a`/`e1a806d`; `3b8eecb` added the native Q6_K reduction/output-layout probe and `be93398` refreshed the sweep. The 2026-05-19 strict device artifact reached native Q6_K with verified writeback and canonical reduction-tree math; the expanded 32-sample probe rejected a simple fixed-offset output layout, and the row provenance probe found no sampled same-row or other-row match. The lane now classifies `q6-native-device-execution-or-final-store`. The active blocker is native final-store/device execution, not bridge writeback/descriptors | Must keep llama.cpp unmodified; diagnostics remain non-promoting until native Q6_K matches and benchmark claims are allowed |
...
| Memory pager fast-gate wiring | Erdos/Main agent | `scripts/verify-fast.sh`, ADB-off queue | Erdos found Task H already claimed `scripts/verify-memory-pager-contract.py` guarded the pager evidence contract, but the verifier and focused unittest were missing from `verify-fast`. The landed follow-up wires both into the fast host gate. | Host-only contract coverage; no device memory-pager promotion |
| Fast gate CI-ledger parity pass | James/Curie/Plato/Main agent | `scripts/verify-fast.sh`, ADB-off queue | James found several CI-ledger fast/static claims were not explicit in `verify-fast`; Curie confirmed adjacent host-only candidates; Plato then identified exact missing ledger commands. The landed follow-up wires OOM/LMK survival, media bridge readiness, no-PRoot runtime-truth, COW overlay kill-at-step, llama GPU memory-blocker classification, and exact pager/service contract unit tests into the fast gate without promoting their device lanes. | Host-only static/synthetic coverage; no ADB/device success claim |
