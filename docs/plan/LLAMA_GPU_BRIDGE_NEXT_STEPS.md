...
| 2026-07-14 VK_KHR_maintenance5 query/index-buffer alias lane | The container-side Vulkan ICD now advertises `VK_KHR_maintenance5`, accepts it during device creation, and exposes the implemented maintenance5 KHR entry points: `vkGetImageSubresourceLayout2KHR`, `vkGetDeviceImageSubresourceLayoutKHR`, `vkGetRenderingAreaGranularityKHR`, and `vkCmdBindIndexBuffer2KHR`. The optional `maintenance5` feature bit remains false-only, Vulkan API version remains capped below 1.4, `vkGetImageSubresourceLayout2EXT` remains hidden, and unsupported maintenance5 semantics still fail closed. No executor ABI, shader bytes, llama.cpp, Dockerfiles, models, or prompts were changed. | `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; host tests `tests.test_gpu_abi_contract tests.test_vulkan_icd_feature_chain`; glibc payload build `scripts/build-gpu-shim.sh`; no llama.cpp/Dockerfile/model/prompt changes |
