...
| 2026-06-04 Vulkan graphics writable storage-buffer descriptor lane | Executor P6 now accepts writable `VK_DESCRIPTOR_TYPE_STORAGE_BUFFER` graphics descriptors for the supported subset.  It materializes the referenced fd-backed buffer ranges as host-visible Android Vulkan storage buffers, records a shader-write to host-read barrier before queue completion, writes the changed descriptor ranges back to the shared backing fd after submit, and emits `vulkan-graphics-v6-storage-buffer-writeback` evidence.  Storage images, input attachments, depth/stencil, MSAA/resolve, and broader synchronization remain fail-closed. | `app/src/main/cpp/pdocker_gpu_executor.c`; host test `tests.test_gpu_abi_contract` |
...
subset, but storage images, input attachments, copy+draw mixed submit semantics,
depth/stencil, MSAA/resolve, and broader synchronization remain fail-closed.  It must not be mixed with llama
