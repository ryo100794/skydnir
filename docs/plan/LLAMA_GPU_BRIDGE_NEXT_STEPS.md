...
| 2026-07-16 V5.6 compute layout replay lane | CPU-side replay now consumes the V5.6 compute layout tables instead of synthesizing a descriptor/pipeline layout from observed descriptor writes. Descriptor-set layouts are recreated from transported binding declarations, descriptor-indexing flags are feature-gated, pipeline-layout set membership is replayed, push-constant ranges come from the original pipeline layout, and the pipeline cache is bypassed for this exact-layout path until cache keys include the full V5.6 layout identity. Dynamic buffer descriptor layouts remain fail-closed because the current compute bind path does not yet transport dynamic-offset arrays into `vkCmdBindDescriptorSets`; this is intentional rather than a silent non-dynamic lowering. | `app/src/main/cpp/pdocker_gpu_executor.c`; `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; host gate `tests.test_gpu_abi_contract`; native gate `scripts/build-gpu-shim.sh`; no llama.cpp/Dockerfile/model/prompt changes |
| 2026-07-16 V5.6 exact descriptor preflight lane | The compute V5.6 exact-layout path now validates transported descriptor writes against the transported layout before creating/writing descriptor sets: dynamic buffer descriptor entries are rejected, duplicate `(set,binding,array)` writes fail, writes outside declared descriptor counts fail, every non-partially-bound descriptor element must be present, update-after-bind does not relax missing-element checks, variable descriptor count and immutable samplers are rejected at the ICD collector, and all-empty/incomplete V5.6 layouts fail closed instead of falling back to synthesized generic layouts. Push-constant replay is constrained to one compute-only range at offset zero until the ABI carries full per-command push operations. | `app/src/main/cpp/pdocker_gpu_executor.c`; `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; host gate `tests.test_gpu_abi_contract`; native gate `scripts/build-gpu-shim.sh`; no llama.cpp/Dockerfile/model/prompt changes |
...
| 2026-07-15 strict compute pipeline layout preflight lane | The container-side ICD now rejects strict compute pipeline creation when the shader module or pipeline layout handle is not tracked, and rejects push-constant layouts that compute V5 cannot replay without ABI extension: multiple ranges, non-zero offsets, non-compute stage flags, or range size drift. | docker-proot-setup/src/gpu/pdocker_vulkan_icd.c; tests.test_gpu_abi_contract; no llama.cpp/Dockerfile/model/prompt changes |
...
| 2026-07-14 Vulkan sampler border-color false-only extension lane | `VK_EXT_custom_border_color` and `VK_EXT_border_color_swizzle` are now advertised as false-only device extensions. Enumeration and create-device extension validation now match the already implemented query surface: custom-border-color and swizzle features remain `VK_FALSE`, `maxCustomBorderColorSamplers` remains zero, fixed-color/no-op sampler pNext chains are accepted, and real custom colors, non-identity swizzles, or sRGB border-color remapping still fail closed. | `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; host tests `tests.test_gpu_abi_contract tests.test_vulkan_icd_feature_chain`; no executor ABI, llama.cpp, Dockerfile, model, prompt, SPIR-V, or shader changes |
...
| 2026-07-14 Vulkan shader layout/memory-model false-only lane | `VK_EXT_scalar_block_layout`, `VK_KHR_uniform_buffer_standard_layout`, `VK_KHR_shader_subgroup_extended_types`, and `VK_KHR_vulkan_memory_model` are now advertised as false-only extension spellings. Their feature pNext structs remain queryable with all feature bits false, all true feature requests still fail closed, and no shader bytes, descriptors, executor ABI, llama.cpp, Dockerfile, model, or prompt are changed. The device-extension enumeration scratch capacity was raised so extension validation cannot silently truncate advertised names as the generic Vulkan surface grows. | `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; host tests `tests.test_gpu_abi_contract tests.test_vulkan_icd_feature_chain`; native build `scripts/build-gpu-shim.sh`; no executor ABI, llama.cpp, Dockerfile, model, prompt, SPIR-V, or shader changes |
| 2026-07-13 Vulkan standalone core feature pNext lane | Standalone core feature structs for multiview, variable pointers, protected memory, shader draw parameters, shader int64 atomics, and imageless framebuffer now participate in feature queries and create-device validation instead of falling into unknown-pNext failure. Multiview mirrors the existing Vulkan 1.1 aggregate advertisement. `VK_KHR_variable_pointers`, `VK_KHR_shader_draw_parameters`, `VK_KHR_shader_atomic_int64`, and `VK_KHR_imageless_framebuffer` are advertised as false-only extension spellings, while their true feature requests still fail closed. | `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; host tests `tests.test_gpu_abi_contract tests.test_vulkan_icd_feature_chain`; no executor ABI, llama.cpp, Dockerfile, model, prompt, or shader changes |
