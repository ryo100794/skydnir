...
| 2026-07-14 VK_KHR_device_group single-device alias lane | The container-side Vulkan ICD now advertises `VK_KHR_device_group`, accepts it during device creation, and exposes `vkGetDeviceGroupPeerMemoryFeaturesKHR`, `vkCmdSetDeviceMaskKHR`, and `vkCmdDispatchBaseKHR` on the existing single-device/no-op implementation. Peer memory features still report zero, device masks are accepted only for the single local device bit, dispatch-base offsets are carried by the existing compute transport, and multi-device pNext/device-mask paths remain fail-closed. | `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; host tests `tests.test_gpu_abi_contract tests.test_vulkan_icd_feature_chain`; glibc payload build `scripts/build-gpu-shim.sh`; no llama.cpp/Dockerfile/model/prompt changes |
...
| 2026-07-14 maintenance5 feature pNext false-only lane | The ICD now recognizes `VkPhysicalDeviceMaintenance5Features{KHR}` in feature-query and create-device pNext chains without advertising `VK_KHR_maintenance5` or Vulkan 1.4. Queries initialize `maintenance5` to false, and create-device accepts the struct only when the requested feature is false; true requests still fail closed with `VK_ERROR_FEATURE_NOT_PRESENT`. This keeps standard pNext chains from being rejected while avoiding any claim that the full maintenance5 feature set or index-buffer2/core-1.4 semantics are available. | `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; host test `tests.test_gpu_abi_contract`; no executor ABI, llama.cpp, Dockerfile, model, prompt, SPIR-V, or shader changes |
| 2026-07-14 Vulkan 1.3 feature aggregate false-only lane | The ICD now recognizes `VkPhysicalDeviceVulkan13Features` in feature-query and create-device pNext chains while continuing to cap the public API version at Vulkan 1.2. Queries zero-preserve the aggregate and leave all 1.3 feature bits false; create-device accepts the aggregate only when every bit is false and rejects any true bit as `vulkan13Features`. This prevents defensive all-zero 1.3 pNext chains from failing without advertising or enabling Vulkan 1.3, maintenance5, or broader 1.3 semantics. | `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; host test `tests.test_gpu_abi_contract`; no executor ABI, llama.cpp, Dockerfile, model, prompt, SPIR-V, or shader changes |
| 2026-07-14 Vulkan 1.4 feature aggregate false-only lane | The ICD now recognizes `VkPhysicalDeviceVulkan14Features` in feature-query and create-device pNext chains while still advertising Vulkan 1.2. Queries zero-preserve the aggregate and leave all 1.4 feature bits false; create-device accepts it only when every bit is false and rejects true requests as `vulkan14Features`. This handles defensive all-zero modern aggregate chains without advertising Vulkan 1.4, maintenance5/6, host-image-copy, push-descriptor, pipeline-robustness, or other unimplemented 1.4 semantics. | `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; host tests `tests.test_gpu_abi_contract tests.test_vulkan_icd_feature_chain`; no executor ABI, llama.cpp, Dockerfile, model, prompt, SPIR-V, or shader changes |
| 2026-07-14 device extension collector parity lane | Device extension enumeration, create-device extension validation, and single-name advertisement checks now share `collect_advertised_device_extensions`. This removes duplicated extension predicates between enumeration and validation, so an advertised extension is accepted by `vkCreateDevice`, unadvertised names still fail closed, and `VK_KHR_maintenance5` remains hidden while its false-only feature pNext is tolerated. | `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; host tests `tests.test_gpu_abi_contract tests.test_vulkan_icd_feature_chain`; glibc payload build `scripts/build-gpu-shim.sh`; no executor ABI, llama.cpp, Dockerfile, model, prompt, SPIR-V, or shader changes |
...
This does not add a barrier ABI yet.  It prevents Skydnir from forwarding
internally contradictory descriptor/layout state and keeps the remaining exact
stage/access-mask gap explicit for the later append-only barrier-table lane.
...
`VK_ERROR_EXTENSION_NOT_PRESENT`, and `VK_KHR_maintenance5` remains unadvertised
...
Skydnir still does not advertise `VK_KHR_maintenance5`, Vulkan 1.4, or the
full maintenance5 feature set.  The ICD now treats
