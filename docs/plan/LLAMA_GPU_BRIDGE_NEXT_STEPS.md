...
the compatibility layer could encode as legacy V4 text.  Strict pass-through
evidence must use the object/hash-validated transport path; the text command
path remains a compatibility lane for non-strict dispatches only.
