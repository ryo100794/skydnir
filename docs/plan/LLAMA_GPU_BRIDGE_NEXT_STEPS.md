...
| 2026-07-15 strict shader-compat rewrite gate lane | Strict Vulkan pass-through now fail-closes SPIR-V and execution-policy compatibility mutation, including LocalSize legalization, specialization materialization, Float16 capability insertion, duplicate descriptor Binding rewrites, Q4/Q6 safe-kernel substitution, Q4 retry specialization materialization, Q6 shader compatibility rewrites, pipeline optimization flag changes, and readonly-overlap data materialization unless the diagnostic plan explicitly sets PDOCKER_GPU_ALLOW_STRICT_SHADER_COMPAT_REWRITES. This separates byte-preserving pass-through evidence from diagnostic rewrite evidence while leaving the Q6 diagnostic overlay explicit. | app/src/main/cpp/pdocker_gpu_executor.c; scripts/llama-gpu-env-manifest.json; tests.test_gpu_abi_contract; no llama.cpp/Dockerfile/model/prompt changes |
| 2026-07-15 strict image-layout replay lane | Strict Vulkan compute and graphics replay now reject image descriptor, rendering attachment, resolve attachment, explicit barrier, clear, copy, resolve, blit, and staged image-upload layout drift before inserting executor-side image layout barriers or updating the replay layout tracker. The bridge may still replay app-provided layout state, but strict pass-through no longer silently corrects descriptor or graphics command layout mismatches inside Android replay. | app/src/main/cpp/pdocker_gpu_executor.c; tests.test_gpu_abi_contract; no llama.cpp/Dockerfile/model/prompt changes |
...
Strict graphics replay now rejects executor-side staged image uploads instead
of using an Android-side staging buffer, transfer barrier, and copy command to
materialize image contents.  Staged upload remains available only in non-strict
diagnostic replay.  This keeps strict pass-through from inventing a copy path
that the container command stream did not explicitly record.
