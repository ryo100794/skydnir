...
primary native classic render-pass scope and inside inherited secondary command
buffers that declare a classic `VkCommandBufferInheritanceInfo::renderPass`.
The producer still rejects inherited dynamic-rendering clear attachments because
that path does not yet persist enough inheritance metadata to validate active
attachments safely.

For inherited classic secondary command buffers, attachment compatibility is
validated against the inherited render pass/subpass at record time.  Clear rects
keep only basic local validation while recording because the active render area
and framebuffer layer count belong to the primary render pass instance.  During
`vkCmdExecuteCommands`, `append_secondary_command_buffer()` now applies the
primary-scope validation before flattening: matching render pass/subpass,
optional framebuffer equality, clear attachment compatibility against the
primary active scope, and clear rect/layer bounds against the primary active
render area and framebuffer/dynamic layer count.

This removes another dynamic-rendering-only producer gate without adding a new
ABI.  The Android executor already validates and replays V6.16 clear rows
against active dynamic or classic render-pass state via `vkCmdClearAttachments`.
This is generic Vulkan pass-through work; it does not change llama.cpp,
Dockerfiles, models, prompts, shader bytes, or arithmetic.
...
passes.  When a subpass cannot be represented as dynamic rendering but the
Android executor advertises V6.34 classic render-pass sideband support,
`vkCmdBeginRenderPass`, `vkCmdNextSubpass`, and `vkCmdEndRenderPass` append
classic BEGIN/NEXT/END marker command rows directly.  Those marker rows do not
require a dynamic-rendering snapshot; the frame sender requires the V6.34
sideband before serializing them, and the executor replays them with native
`vkCmdBeginRenderPass`/`vkCmdNextSubpass`/`vkCmdEndRenderPass`.

Graphics pipeline capture no longer marks a non-normalizable classic render-pass
pipeline unsupported solely because it cannot be converted to dynamic rendering.
It now allows that case only when the strict V6.34 replay contract is complete
for the render pass.  Draw recording also treats active native classic
render-pass scope as a valid graphics rendering context instead of rejecting it
just because `cmd->render_pass_active` is true.
...
- `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest -q tests.test_gpu_abi_contract.GpuAbiContractTest.test_vulkan_begin_render_pass_normalizes_subpass_to_dynamic_rendering tests.test_gpu_abi_contract.GpuAbiContractTest.test_vulkan_next_subpass_normalizes_to_end_barrier_begin_sequence tests.test_gpu_abi_contract.GpuAbiContractTest.test_vulkan_classic_render_pass_native_replay_is_not_dynamic_rendering_only tests.test_gpu_abi_contract.GpuAbiContractTest.test_vulkan_render_pass_pipeline_formats_are_completed_from_attachment_refs tests.test_gpu_abi_contract.GpuAbiContractTest.test_vulkan_graphics_v632_render_pass_transport_does_not_claim_strict_by_normalization`
- `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest -q tests.test_gpu_abi_contract`
- `bash scripts/build-gpu-shim.sh`
- `PYTHONDONTWRITEBYTECODE=1 python3 scripts/verify-native-payloads.py`
- `PYTHONDONTWRITEBYTECODE=1 python3 scripts/verify-docs-maintenance.py`
...
| 2026-07-20 dynamic-rendering local-read false-feature negotiation lane | `VK_KHR_dynamic_rendering_local_read` is now advertised and accepted only as a false-feature negotiation surface. `VkRenderingAttachmentLocationInfo` remains accepted only when it is a true identity/no-op mapping for active dynamic-rendering color attachments. `dynamicRenderingLocalRead` stays `VK_FALSE`; true feature requests, local-read extension enable without `VK_KHR_dynamic_rendering`, and non-identity input-attachment local-read payloads fail closed. The local-read command proc surface is present but hidden from device proc lookup unless the feature+extension mask is enabled, which cannot happen yet through public feature validation. | `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; `tests.test_gpu_abi_contract`; `tests.test_vulkan_icd_feature_chain`; glibc payload build `scripts/build-gpu-shim.sh`; no executor ABI, llama.cpp, Dockerfile, model, prompt, SPIR-V, or shader changes |
...
| 2026-07-20 Vulkan sampler border-color false-feature negotiation lane | `VK_EXT_custom_border_color` and `VK_EXT_border_color_swizzle` are advertised and accepted for standard extension negotiation while their feature bits remain `VK_FALSE` and `maxCustomBorderColorSamplers` remains zero. Fixed-color/no-op sampler pNext chains are accepted, including identity border-color component mapping with `srgb == VK_FALSE`; actual custom colors, non-identity swizzles, and sRGB border-color remapping still fail closed because the sampler ABI does not transport those payloads. | `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; host tests `tests.test_gpu_abi_contract tests.test_vulkan_icd_feature_chain`; native build `scripts/build-gpu-shim.sh`; no executor ABI, llama.cpp, Dockerfile, model, prompt, SPIR-V, or shader changes |
...
| 2026-07-20 VK_EXT_memory_priority validated hint lane | The ICD advertises `VK_EXT_memory_priority`, reports `VkPhysicalDeviceMemoryPriorityFeaturesEXT.memoryPriority = VK_TRUE`, accepts true/false feature requests during device creation, and accepts `VkMemoryPriorityAllocateInfoEXT` values in the Vulkan priority range `0.0..1.0`. Priority remains an allocation-residency hint consumed at the ICD boundary; it does not add executor ABI state, shader changes, llama.cpp changes, Dockerfile changes, model changes, or prompt changes. Out-of-range priority values and unknown allocation pNext structs remain fail-closed. | `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; host tests `tests.test_gpu_abi_contract tests.test_vulkan_icd_feature_chain`; glibc payload build `scripts/build-gpu-shim.sh`; no executor ABI, llama.cpp, Dockerfile, model, prompt, or shader changes |
...
`memoryPriority = VK_TRUE`.  `vkCreateDevice` accepts the feature struct with
either true or false because `VK_EXT_memory_priority` is advertised by the ICD.
`vkAllocateMemory` classifies `VkMemoryPriorityAllocateInfoEXT` and accepts
priority values in the Vulkan allocation-priority hint range `0.0..1.0`.  The
priority is consumed as ICD-local residency metadata and is not serialized into
the Android executor command ABI.  Values outside `0.0..1.0` and unknown
allocation pNext structs remain fail-closed so the bridge does not silently
reinterpret invalid application state.
...
metadata.  `VK_EXT_custom_border_color` and `VK_EXT_border_color_swizzle` are
now advertised only as false-feature negotiation surfaces: no-op sampler pNext
chains are accepted, while custom border color payloads, non-identity component
swizzles, and sRGB remapping remain fail-closed.  Shader layout and
memory-model extensions are still gated behind a false transport helper because
they imply SPIR-V/layout semantics that are not yet validated or represented by
the executor.  `VK_KHR_shader_float_controls` stays advertised with
conservative all-false properties, and the Vulkan 1.2
...
### 2026-07-19 CPU/static Vulkan ABI ID/index classification gate

The CPU-only pass-through hardening lane now has a regression gate for Vulkan
wire ABI identifier and table-index fields. `tests/test_gpu_abi_contract.py`
extracts every integer field ending in `_id` or `_index` from the typed Vulkan
V5/V6 structs, plus legacy V4 positional binding IDs, then requires a
struct-qualified classification before the field can cross the glibc ICD -> APK
executor boundary.

The ID classifications separate correlation IDs, native handle transport IDs,
...
IDs. The index classifications separate fd arrays, command rows, descriptor
rows, resource/image/image-view/sampler/pipeline rows, identity-key ordinals,
queue-family indices, draw parameters, and submit/device references. The test
also pins the executor-side guard families for resource, image, image-view,
sampler, buffer-view, pipeline, layout, native-handle identity, fd bounds,
command/resource/image/pipeline row bounds, queue-family replayability, and
...
class of pass-through regressions where a new handle-like or row-selecting ABI
