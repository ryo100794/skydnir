...
| 2026-07-15 graphics V6.24 descriptor-layout cap explicit lane | Graphics V6.24 descriptor-set-layout metadata now fails closed against its own per-set binding cap instead of the unrelated storage-buffer constant, and Android frame validation rejects frames whose V6.24 table carries more than the declared per-set binding count for a layout. This does not claim full heap-backed V6.30 layout transport yet; it removes an implicit/truncation-prone boundary and makes the remaining V6.24 limit explicit. | `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; `app/src/main/cpp/pdocker_gpu_executor.c`; host test `tests.test_gpu_abi_contract`; no llama.cpp/Dockerfile/model/prompt changes |
| 2026-07-15 V5 FD capability advertising cleanup lane | Android executor capability JSON now advertises `max_fds` as the effective single-message transport cap and exposes the wider 253-value ABI index range separately as `abi_fd_index_range`. This removes the misleading appearance that one V5 frame can currently pass 253 FDs; the existing sender/receiver fail-closed checks still use `PDOCKER_GPU_TRANSPORT_MAX_PASSED_FDS` until chunked FD-table transport is implemented. | `app/src/main/cpp/pdocker_gpu_executor.c`; host test `tests.test_gpu_abi_contract`; native executor build; no llama.cpp/Dockerfile/model/prompt changes |
