...
| 2026-06-15 Vulkan graphics V6.14/V6.15 advertised-cap gate lane | V6.14 resolve and V6.15 blit metadata remain append-only, but replay is now gated by the same capabilities the ICD advertises. Resolve requires advertised multisample-image support, source MSAA, destination single-sample, matching color format, transfer usage, and color-attachment-capable format features. Current advertised image properties expose only `VK_SAMPLE_COUNT_1_BIT`, so resolve fails closed. Blit requires single-sample color images plus advertised `BLIT_SRC`/`BLIT_DST` format features, and `VK_FILTER_LINEAR` additionally requires `SAMPLED_IMAGE_FILTER_LINEAR`; current format properties intentionally advertise none of those bits, so blit fails closed. Future widening requires executor-derived capability advertisement instead of guessed caps. | `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; `app/src/main/cpp/pdocker_gpu_executor.c`; host test `tests.test_gpu_abi_contract`; native builds `scripts/build-gpu-shim.sh`, `scripts/build-native-android-ndk.sh`; APK gate `:app:assembleDebug`; no llama.cpp changes |
...
- **V6.14** serializes command-ordered `vkCmdResolveImage`; replay is
  currently fail-closed unless advertised caps include multisample resolve
  support.
...
multiplanar/compressed images, copy2 pNext payloads, MSAA resolve/blit
capability advertisement, resolve/blit while inside dynamic rendering,
...
view gives a bounded single-aspect color, pure depth (`D16`/`D32`), or pure
stencil (`S8`) mip/layer range; storage-image descriptors remain color-only,
and packed dual-aspect depth/stencil descriptors remain fail-closed.
