...
| 2026-07-15 strict image-layout replay lane | Strict Vulkan compute and graphics replay now reject image descriptor, rendering attachment, and resolve attachment layout drift before inserting executor-side image layout barriers. The bridge may still replay app-provided layout state, but strict pass-through no longer silently corrects descriptor or rendering layout mismatches inside Android replay. | app/src/main/cpp/pdocker_gpu_executor.c; tests.test_gpu_abi_contract; no llama.cpp/Dockerfile/model/prompt changes |
