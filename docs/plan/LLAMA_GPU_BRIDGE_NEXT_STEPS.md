...
| 2026-07-06 Vulkan descriptor/layout early-reject lane | The container ICD now aligns descriptor/pipeline layout creation with the conservative advertised feature contract before objects are created. Descriptor set layouts reject duplicate bindings, zero descriptor counts, immutable samplers, unsupported binding flags, and oversized arrays through the same support/create path. Pipeline layouts now fail early for oversized set-layout counts, invalid set-layout handles, missing push-constant ranges, and unsupported push-constant range counts instead of creating handles that fail only at bind/submit time. This prevents false compatibility while descriptor indexing and immutable-sampler transport remain intentionally non-advertised. | `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; host tests `tests.test_gpu_abi_contract tests.test_llama_gpu_env_parity tests.test_llama_gpu_readiness_contract`; native build `scripts/build-gpu-shim.sh`; fast gate `scripts/verify-fast.sh`; no llama.cpp/Dockerfile/model/prompt changes |
