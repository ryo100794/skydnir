...
| 2026-06-05 Vulkan graphics depth/stencil disabled-state attachment lane | The executor now replays depth/stencil attachments for the conservative subset where the serialized graphics pipeline keeps depth/stencil tests and writes disabled and the attachment store op is `DONT_CARE`.  It validates depth/stencil attachment layouts and image usage, materializes the image view, records depth/stencil dynamic-rendering attachment pointers, and creates a matching Android graphics pipeline with depth/stencil formats but disabled `VkPipelineDepthStencilStateCreateInfo`.  Depth/stencil writes/readback, enabled depth/stencil tests, MSAA/resolve, mixed submit semantics, and broader synchronization remain fail-closed. | `app/src/main/cpp/pdocker_gpu_executor.c`; host test `tests.test_gpu_abi_contract`; native build `scripts/build-native-android-ndk.sh` |
...
semantics, enabled depth/stencil tests or depth/stencil writeback,
