...
| 2026-07-15 strict shader-compat rewrite gate lane | Strict Vulkan pass-through now fail-closes Q4/Q6 safe-kernel substitution and Q6 shader compatibility rewrites unless the diagnostic plan explicitly sets PDOCKER_GPU_ALLOW_STRICT_SHADER_COMPAT_REWRITES. This separates byte-preserving pass-through evidence from llama-oriented diagnostic rewrite evidence while leaving the Q6 diagnostic overlay explicit. | app/src/main/cpp/pdocker_gpu_executor.c; scripts/llama-gpu-env-manifest.json; tests.test_gpu_abi_contract; no llama.cpp/Dockerfile/model/prompt changes |
