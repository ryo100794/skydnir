...
| 2026-07-14 compute V5 buffer usage identity lane | Generic compute V5/V5.1 transport now carries the original container-side `PdockerVkBuffer::usage` through `PdockerGpuVulkanDispatchV5ResourceEntry::usage`, materializes it into `VulkanDispatchBinding::api_buffer_usage`, and uses that API usage as the Android strict-object-graph buffer-usage base while OR-ing only replay-required storage/transfer/indirect/texel bits. This removes a compute object-identity narrowing point where usage was reconstructed solely from descriptor observations. | `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; `app/src/main/cpp/pdocker_gpu_executor.c`; host test `tests.test_gpu_abi_contract`; no llama.cpp/Dockerfile/model/prompt changes |
...
the existing V5 resource `usage` field, the Android executor materializes it into
`VulkanDispatchBinding::api_buffer_usage`, and strict object-graph replay uses it
as the base `VkBufferCreateInfo::usage` before adding the replay-required storage,
transfer, indirect, or texel bits.
