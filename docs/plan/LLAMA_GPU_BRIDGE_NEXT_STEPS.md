...
| 2026-06-04 Vulkan graphics explicit image-barrier lane | V6.1 now carries explicit image barriers as a separate extension table.  The ICD serializes `vkCmdPipelineBarrier` / `vkCmdPipelineBarrier2` image barriers into ordered graphics commands, and the executor validates subresource ranges, rejects queue-family ownership transfers, and records matching Android `vkCmdPipelineBarrier` calls in command order.  Buffer/memory barriers, storage/input images, write descriptors, depth/stencil, MSAA/resolve, and broader synchronization remain fail-closed. | `app/src/main/cpp/pdocker_gpu_abi.h`; `docker-proot-setup/src/gpu/pdocker_gpu_abi.h`; `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; `app/src/main/cpp/pdocker_gpu_executor.c`; host tests `tests.test_gpu_abi_contract tests.test_llama_gpu_env_parity`; native builds `scripts/build-native-android-ndk.sh`, `scripts/build-gpu-shim.sh`; APK gate `:app:assembleCompatDebug` |
