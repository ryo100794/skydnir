...
| 2026-07-12 ICD buffer-view state groundwork | The container-side Vulkan ICD now tracks `VkBufferView` handles, validates buffer-view usage/format/range, and shadows uniform/storage texel-buffer descriptor writes/copies as distinct descriptor slot state. This is deliberately not advertised as executable texel-buffer support yet: generic compute and graphics submit still fail closed with `texel-buffer descriptor ABI pending` because the current V5/V6 descriptor-object ABI has no append-only buffer-view metadata table carrying `VkFormat`, offset, range, and view identity to the Android executor. | `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; host test `tests.test_gpu_abi_contract`; no executor ABI change; no llama.cpp/Dockerfile/model/prompt changes |
