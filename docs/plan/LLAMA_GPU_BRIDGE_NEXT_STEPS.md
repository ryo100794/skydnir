...
| 2026-07-04 headless WSI device-group no-op lane | `vkAcquireNextImage2KHR` and `vkQueuePresentKHR` now accept single-device `VkDeviceGroupAcquireNextImageInfoKHR` / `VkDeviceGroupPresentInfoKHR` pNext metadata as no-op for the headless swapchain path. Multi-device masks, non-local present modes, mismatched swapchain counts, and unknown WSI pNext chains remain fail-closed. | `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; host test `tests.test_gpu_abi_contract`; no llama.cpp/Dockerfile/model/prompt changes |
