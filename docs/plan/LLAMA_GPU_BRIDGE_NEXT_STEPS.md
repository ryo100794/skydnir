...
| 2026-07-11 V5 compute native-plan gate | The Android executor now builds a `VulkanDispatchV5NativePlan` from V5/V5.1/V5.2 resource, descriptor, image, view, sampler, and V5.2 layout-range tables before any legacy V4 binding conversion. Frames that exceed the legacy 16-slot execution fallback are explicitly rejected as native-table replay work instead of being narrowed through `convert_vulkan_dispatch_v5_to_v4_bindings`. This is not full table-native replay yet; the next slice must replace the fallback with heap-backed descriptor/object arenas and widen the ICD producer. | `app/src/main/cpp/pdocker_gpu_executor.c`; host test `tests.test_gpu_abi_contract`; native build `scripts/build-native-android-ndk.sh`; no llama.cpp/Dockerfile/model/prompt changes |
...
| 2026-07-11 executor V5 handler heap tables | The Android executor V5 handler now allocates its legacy-conversion binding, image-descriptor, and fd tables from the already validated `VulkanDispatchV5NativePlan` counts instead of fixed `PDOCKER_GPU_MAX_VULKAN_BINDINGS` stack arrays. This removes another executor-side stack/narrowing point before the V4 fallback. It is still not full V5 table-native replay: frames that require non-legacy descriptor-table execution remain fail-closed until `run_vulkan_dispatch_fd` is replaced or widened. | `app/src/main/cpp/pdocker_gpu_executor.c`; host test `tests.test_gpu_abi_contract`; native build `scripts/build-native-android-ndk.sh`; no llama.cpp/Dockerfile/model/prompt changes |
...
| 2026-07-04 Vulkan sampler reduction pNext lane | Sampler creation now accepts `VkSamplerReductionModeCreateInfo` only for the default `VK_SAMPLER_REDUCTION_MODE_WEIGHTED_AVERAGE` mode, which is equivalent to the base sampler behavior already transported to Android Vulkan. Min/max reduction remains fail-closed because `samplerFilterMinmax` is not advertised and the V5 sampler table does not carry a promoted reduction mode. Unknown sampler pNext structs still fail closed. | `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; host test `tests.test_gpu_abi_contract`; no llama.cpp/Dockerfile/model/prompt changes |
...
| 2026-07-03 Vulkan P1 core fail-closed surface lane | The ICD now exposes additional generic core Vulkan entry points needed by non-llama Vulkan 1.0/1.1/1.2 clients while keeping unsupported capabilities conservative: legacy sparse image requirements report zero, sparse queue binds reject non-empty work, buffer views / sampler YCbCr conversions / descriptor update templates fail closed with null handles, buffer-device-address queries return zero, and command-pool trim is a no-op. KHR/EXT aliases remain hidden when their extensions are not advertised. | `docker-proot-setup/src/gpu/pdocker_vulkan_icd.c`; host test `tests.test_gpu_abi_contract`; native build `scripts/build-gpu-shim.sh`; no llama.cpp/Dockerfile/model/prompt changes |
