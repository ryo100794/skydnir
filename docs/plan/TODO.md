...
- [todo] **Vulkan graphics texel-buffer ABI lane**: Extend graphics V6 with an append-only buffer-view metadata table carrying descriptor identity, `VkFormat`, offset, range, and buffer-view generation/resource mapping. Compute V5.3 now transports and materializes texel-buffer descriptors, and texel buffer `bufferFeatures` are gated on executor V5.3 support. Graphics V6 remains pending until the same explicit metadata and executor materialization are proven for graphics replay.
