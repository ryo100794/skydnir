...
- [done] **V6.1 image-barrier VK_REMAINING/aspect slice**: V6.1 explicit image
  barriers now normalize `VK_REMAINING_MIP_LEVELS` and
...
- [done] **Q6 final-store SSA value-path lane**: `analyze-spirv.py` now records bounded producer-chain, Function-store expansion, dependency-frontier, unresolved-id, and truncation evidence for each final binding-2 Q6 store. `compare-spirv-dataflow.py` compares that path so SSA value-flow drift is caught as a CPU/static mismatch. Evidence: `python3 -m unittest tests.test_gpu_abi_contract.GpuAbiContractTest.test_spirv_analyzer_follows_q6_function_accumulator_final_store_flow -q` and `python3 -m unittest tests.test_gpu_abi_contract.GpuAbiContractTest.test_spirv_dataflow_compare_reports_q6_final_store_flow_paths -q`.
- [done] **Q6 output-index SSA value-path lane**: final binding-2 Q6 stores now carry `output_index_ssa_path` beside `ssa_value_path`, preserving WorkgroupId/NumWorkgroups, push-constant, specialization, unresolved-id, and truncation evidence for the destination index expression. The dataflow comparator includes it so row/layout drift is an exact static mismatch. Evidence: `python3 -m unittest tests.test_gpu_abi_contract.GpuAbiContractTest.test_spirv_analyzer_follows_q6_function_accumulator_final_store_flow -q` and `python3 -m unittest tests.test_gpu_abi_contract.GpuAbiContractTest.test_spirv_dataflow_compare_reports_q6_final_store_flow_paths -q`.
- [done] **Q6 SSA boundary classification lane**: `compare-spirv-dataflow.py` now classifies isolated stored-value SSA drift as `q6-final-store-ssa-value` and isolated output-index SSA drift as `q6-output-index-ssa` instead of the broader final-store value-flow bucket. Evidence: `python3 -m unittest tests.test_gpu_abi_contract.GpuAbiContractTest.test_spirv_dataflow_compare_reports_q6_final_store_flow_paths -q`.
- [done] **Q6 value-flow sub-boundary lane**: final-store value-flow classification now scans all diff paths, emits `sub_boundaries`, and reports `q6-final-store-value-flow-mixed` when stored-value arithmetic, stored-value SSA, output-index SSA, control, dependency, or structural drifts appear together. Evidence: `python3 -m unittest tests.test_gpu_abi_contract.GpuAbiContractTest.test_spirv_dataflow_compare_reports_q6_final_store_flow_paths -q`.
...
- [done] **image view flags executor boundary gate**:
  Image-view flags are now fail-closed on both sides of the bridge. The ICD
  rejects nonzero `VkImageViewCreateInfo::flags`; the Android executor now
  independently rejects transported image-view entries with nonzero `flags`
...
- [done] **sampler scalar executor boundary validator**:
  Transported sampler scalar fields are now validated before native
  `vkCreateSampler` replay. The executor rejects unsupported filter, mipmap,
  address, compare-op, and border-color enum values, malformed `VkBool32`
  fields, non-finite LOD values, and `minLod > maxLod`; mirror-clamp address
...
- [done] **compute V5.3 texel-buffer descriptor lane**: The container-side Vulkan ICD now creates and destroys tracked `VkBufferView` objects, stores buffer usage on `VkBuffer`, validates buffer-view format/range/use at `vkCreateBufferView`, and shadows uniform/storage texel-buffer descriptor writes as `buffer_view` descriptor slots. Compute dispatch V5 adds an append-only V5.3 buffer-view metadata table carrying descriptor identity, `VkFormat`, offset, range, and generation to the Android executor. The executor validates that metadata, creates native `VkBufferView` objects, writes `pTexelBufferView` descriptors, keeps the views alive for the submit, and destroys them during cleanup.
...
- [done] **Vulkan graphics texel-buffer ABI lane**: Graphics V6 now has an append-only V6.27 buffer-view metadata table carrying command/descriptor identity, resource mapping, `VkFormat`, offset, range, buffer-view id, and generation. The container-side ICD accepts graphics texel-buffer layouts only when the executor advertises V6.27 support, serializes texel descriptors as normal buffer resources plus explicit buffer-view metadata, and selects the V6.27 frame header when required. The Android executor validates the V6.27 table against descriptor/resource ranges, creates native `VkBufferView` objects, writes `pTexelBufferView` descriptors, preserves view lifetime through replay, and destroys them during cleanup. Verified with `python3 -m unittest tests.test_gpu_abi_contract -q`, `bash scripts/build-gpu-shim.sh`, `bash scripts/build-native-android-ndk.sh`, `./gradlew :app:compileCompatDebugKotlin`, and `bash scripts/verify-fast.sh` on 2026-07-12.
- [done] **descriptor image-layout fail-closed CPU harness lane**: `tests/test_vulkan_icd_sync_harness.py` now compiles a host C harness that drives `descriptor_image_layout_matches_tracked_state` and `send_generic_vulkan_dispatch_op` with minimal generic Vulkan structs. It proves sampled image descriptors accept the tracked read-only layout, mismatches reject with `-EOPNOTSUPP` before transport, storage images require `VK_IMAGE_LAYOUT_GENERAL`, and mixed explicit layout ranges must cover the full view without conflicts. Evidence: `python3 -m unittest tests.test_vulkan_icd_sync_harness.VulkanIcdSyncHarnessTest.test_descriptor_image_layout_mismatch_fails_before_transport`; ABI subset: `python3 -m unittest tests.test_gpu_abi_contract.GpuAbiContractTest.test_vulkan_dispatch_icd_fails_closed_on_descriptor_image_layout_mismatch`.
- [doing] **V5 compute native descriptor-table execution**: Compute V5/V5.1/V5.2
  ...
  descriptor state.  The Android graphics V6 descriptor materializer now stages
...
- [done] Root script clutter now has a first-pass inventory gate:
  ...
  the former `scripts/verify-llama-startup-logging.py`, or the former
  ...
  startup-json, and KV-offload guard assertions. The box64 NDK wrapper was
  later retired after `scripts/build-native-android-ndk.sh`,
  `docs/build/NATIVE_BUILD_ENVIRONMENT.md`, and native-build ABI tests confirmed
  the supported path no longer depends on mutating NDK tools. The remaining
  terminal repro script stays retained as `obsolete-suspect` until fresh paired
  UI self-test and Engine exec-input JSONL artifacts fully replace it.
