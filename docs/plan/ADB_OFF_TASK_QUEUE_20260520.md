...
ADBOFF-001 through ADBOFF-041 have landed. Keep this table as the
...
| ADBOFF-002 | P0 | Keep design/TODO/API boundaries synchronized. | `docs/plan/TODO.md`, compatibility docs, and design docs list Docker API subset, Skydnir extensions, unsupported OCI features, and Bluetooth/BLE/GPS future broker scope. | Done in `eefae1d`. |
...
| ADBOFF-006 | P1 | Maintain this ADB-off queue and plan index. | This document remains linked from `docs/plan/README.md` and referenced by the coordination ledger. | Ongoing maintenance; current queue items ADBOFF-001 through ADBOFF-041 have landed. |
...
| ADBOFF-008 | P1 | Add static Skydnir extension API boundary guard. | Docs distinguish Docker-standard `GET /system/df` and `POST /system/prune` from Skydnir-only `/system/*` routes, and every public `Pdocker*` field observed in pdockerd is documented. | Done in `50edf2f`. |
...
| ADBOFF-040 | P1 | Guard the llama Q6_K workflow compare argv. | `tests.test_llama_gpu_q6k_workflow` now drives the ready path with faked host-only steps and asserts the compare command starts with `bash scripts/android-llama-gpu-compare.sh`, never `bash bash ...`; the module is wired into `scripts/verify-fast.sh`. | Done in this slice. |
| ADBOFF-041 | P1 | Wire recovered host-only contract tests into the fast gate. | `scripts/verify-fast.sh` now runs the memory-layer UI contract, COW kill-at-step host/device-shape contract, GPU ABI contract, and syntax-checks the COW device runner shell; `docs/test/CI_GATE_LEDGER.md` records the same non-promoting fast/static commands. | Done in this slice. |
