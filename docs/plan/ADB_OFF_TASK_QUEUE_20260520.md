...
| ADBOFF-013 | P1 | Refresh or rebuild stale APK-native payload evidence before using `verify-fast` as a full green signal. | `verify-fast` currently reaches `verify-native-payloads.py` and detects the already-built compat APK has an older `assets/pdockerd/pdockerd` than `docker-proot-setup/bin/pdockerd`; either rebuild the APK or refresh the evidence in a dedicated build slice. | Planned. |
