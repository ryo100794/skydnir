...
| Build | 9 top-level scripts | Build and setup helpers are flat but understandable. Public entrypoints are already named in `scripts/README.md`. The standard native helper path is now `scripts/build-native-android-ndk.sh`; `scripts/build-native-termux.sh` remains a legacy local helper. | Keep `build-all.sh`, `build-apk.sh`, `build-gpu-shim.sh`, `build-native-android-ndk.sh`, and `build-native-termux.sh` stable. Move only helper implementations after docs and CI references point at wrappers or new paths. |
| Test | 76 top-level scripts | Most flat files are verification gates, smoke scripts, benchmark runners, or artifact validators. The native payload verifier and native rebuild release verifier are explicit release-hygiene gates, including the F-Droid no-crane dry-run path. | Prefer manifest-backed lanes through `scripts/pdocker-test-driver.py`. Add new verifiers to `scripts/script-inventory.json` first, then decide whether they remain top-level wrappers or move under `scripts/test/`. |
...
| Unused or legacy candidates | 3 tracked candidates | Current candidates remain `android-terminal-it-repro.sh`, `verify-llama-startup-logging.py`, and `wrap-ndk-box64.sh`. They are weakly referenced but not safe to delete in a broad cleanup pass. | Audit each candidate in a focused change. Delete or archive only after replacement commands, docs, and test coverage are confirmed. |
...
| Test runbooks and evidence | [`../test/README.md`](../test/README.md) and topic gate docs | `*-latest.*`, large llama/GPU artifacts, and run directories are easy to mistake for duplicates. | Add topic evidence indexes before pruning or moving artifacts; preserve producer/consumer contracts for latest pointers. |
