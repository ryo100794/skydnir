...
| Unused or legacy candidates | 2 tracked candidates | Current candidates remain `android-terminal-it-repro.sh` and `wrap-ndk-box64.sh`. They are weakly referenced but not safe to delete in a broad cleanup pass. The retired llama startup helper is now covered by `tests/test_llama_startup_logging_contract.py`. | Audit each candidate in a focused change. Delete or archive only after replacement commands, docs, and test coverage are confirmed. |
