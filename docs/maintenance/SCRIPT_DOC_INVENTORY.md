...
| Build | 8 top-level scripts | Build and setup helpers are flat but understandable. Public entrypoints are already named in `scripts/README.md`. | Keep `build-all.sh`, `build-apk.sh`, `build-gpu-shim.sh`, and `build-native-termux.sh` stable. Move only helper implementations after docs and CI references point at wrappers or new paths. |
| Test | 74 top-level scripts | Most flat files are verification gates, smoke scripts, benchmark runners, or artifact validators. Two no-proot runtime truth scripts were present on disk but missing from the machine-readable inventory and are now recorded there. | Prefer manifest-backed lanes through `scripts/pdocker-test-driver.py`. Add new verifiers to `scripts/script-inventory.json` first, then decide whether they remain top-level wrappers or move under `scripts/test/`. |
