...
|---|---:|---|---|---|---|---|
| Service truth: UI/service health must use the same Engine container ID as runtime state, listener owner, and logs. | P0 | **Unmet planned gap.** Static plan gate passes only if the acceptance contract exists; focused device lane must write `Status: planned-gap`, `Success: false` until same-ID proof exists. | `python3 scripts/verify-service-truth-plan.py` | `bash scripts/verify-heavy.sh --android-service-truth` (or direct `bash scripts/android-device-smoke.sh --service-truth <default-workspace\|llama>`) | `docs/test/service-truth-latest.json` | One proof reduces UI card, `/containers/json`, `state.json`, process tree, listener probe, and logs to the same Engine container ID. |
| Runtime teardown: stop/kill/rm must prove process-tree and executor cleanup, not only HTTP 204. | P0 | **Unmet planned gap.** Static contract and promoted-artifact reducer exist; focused device lane remains non-promoting until it collects a full same-container-ID evidence directory and the reducer accepts `Status=device-pass`, `Success=true`. | `python3 scripts/verify-service-truth-plan.py` and `python3 scripts/verify-runtime-teardown-artifact.py --expect-planned-gap docs/test/runtime-teardown-latest.json` | `bash scripts/verify-heavy.sh --android-runtime-teardown` (or direct `bash scripts/android-device-smoke.sh --runtime-teardown <default-workspace\|llama>`) | `docs/test/runtime-teardown-latest.json` plus `docs/test/runtime-teardown/*.json` | Stopped/removed container ID has no listener, no persisted running state, and no orphan `pdocker-direct` / service / GPU executor residue. |
...
| Single-container execution: `docker run --rm ubuntu:22.04 echo hi` must execute inside a container, not host shell or metadata-only mode. | P0 | **Unmet planned gap.** A focused smoke route and strict host artifact verifier now exist, but the row cannot promote until the verifier accepts a real Android device artifact. | `python3 scripts/verify-runtime-single-container-artifact.py <artifact>` plus `python3 -m unittest tests.test_runtime_single_container_artifact_verifier` | `bash scripts/verify-heavy.sh --android-single-container` | `docs/test/runtime-single-container-echo-hi-latest.json` | Artifact proves real Engine container create/start/wait/removal, stdout exactly `hi`, exit code `0`, current 64-hex container ID from `docker --cidfile`, linked device stdout/stderr/log/cidfile evidence, and no host-shell fallback. |
| Modern/no-PRoot runtime truth: metadata-only flavors must not expose execution claims or fake successful `RUN`/`docker run`/Compose services. | P0 | **Blocked for modern execution claims.** Release readiness requires either a complete no-PRoot executor or explicit capability fail-closed UI/API behavior. The current latest artifact is non-promoting planned-gap evidence. | `python3 scripts/verify-release-readiness.py` and `python3 scripts/verify-ui-actions.py` | Device run of the modern flavor, if shipped, must exercise disabled execution actions and explicit capability diagnostics. | `docs/test/no-proot-runtime-truth-latest.json` | Execution actions are hidden/disabled or return structured capability errors; no metadata-only route claims container process execution, published ports, or service health. |
...
- `unittest-all`, which includes terminal `-it`, service-truth artifact, memory
  pager, memory-layer UI, COW kill-at-step, GPU ABI, image-pull, and llama
