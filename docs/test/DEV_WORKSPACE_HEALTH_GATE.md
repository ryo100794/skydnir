...
- UI-triggered compose/build/run produced a running `pdocker-dev` container.
- `/containers/json?all=1` and `/containers/<id>/json` agree on the current
  Engine container ID, name, and running state for `pdocker-dev`.
...
- The rendered project-library service card export is current, not stale,
  unknown, or ambiguous, and its Engine container ID matches `pdocker-dev`.
