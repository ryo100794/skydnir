...
| Image object shape | image format/type/usage/tiling/create flags, image-view flags/component swizzles, and subresource ranges | native `VkImage`/`VkImageView` replay | ICD and executor both allow only optimal-tiling images, ABI-modeled image create flags, zero image-view flags, and valid component swizzle enums; compressed, multiplanar, YCbCr, linear tiling, disjoint, extended-usage, block-texel reinterpretation, sparse, image-view flags, invalid swizzles, and unmodeled mutable-format reinterpretation fail closed until represented in the ABI | image table, image view table, image-format query failures, `image-create-shape-unsupported`, `image-view-component-swizzle-unsupported` |
| Sampler object shape | sampler flags, filters, address modes, LOD, anisotropy, reduction mode, and compare state | native `VkSampler` replay | ICD and executor both reject nonzero sampler flags before replay; feature-gated sampler reduction and anisotropy remain explicit runtime gates | sampler table, sampler replay rejection reason |
