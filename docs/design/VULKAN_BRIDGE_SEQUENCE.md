...
| Image object shape | image format/type/usage/tiling/create flags, image-view component swizzles, and subresource ranges | native `VkImage`/`VkImageView` replay | ICD and executor both allow only optimal-tiling images, ABI-modeled image create flags, and valid component swizzle enums; compressed, multiplanar, YCbCr, linear tiling, disjoint, extended-usage, block-texel reinterpretation, sparse, invalid swizzles, and unmodeled mutable-format reinterpretation fail closed until represented in the ABI | image table, image view table, image-format query failures, `image-create-shape-unsupported`, `image-view-component-swizzle-unsupported` |
