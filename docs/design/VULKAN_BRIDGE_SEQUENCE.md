...
| Image object shape | image format/type/usage/tiling/create flags, single-queue sharing mode, image-view flags/component swizzles, and subresource ranges | native `VkImage`/`VkImageView` replay | ICD and executor both allow only optimal-tiling images, ABI-modeled image create flags, zero image-view flags, valid component swizzle enums, and exclusive single-queue image sharing at the executor boundary; the ICD normalizes valid single-advertised-family concurrent sharing to exclusive before transport. Compressed, multiplanar, YCbCr, linear tiling, disjoint, extended-usage, block-texel reinterpretation, sparse, image-view flags, invalid swizzles, native queue-family expansion, and unmodeled mutable-format reinterpretation fail closed until represented in the ABI | image table, image view table, image-format query failures, `image-create-shape-unsupported`, `image-sharing-mode-unsupported`, `image-view-component-swizzle-unsupported` |
