...
| Image object shape | image format/type/usage/create flags and subresource ranges | native `VkImage`/`VkImageView` replay | ICD and executor both allow only ABI-modeled image create flags; compressed, multiplanar, YCbCr, disjoint, extended-usage, block-texel reinterpretation, sparse, and unmodeled mutable-format reinterpretation fail closed until represented in the ABI | image table, image view table, image-format query failures, `image-create-shape-unsupported` |
