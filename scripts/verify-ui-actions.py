...
def main() -> int:
    ...
    require("runtime flavors have distinct launcher labels", "Skydnir compat" in (ROOT / "app/src/compat/res/values/strings.xml").read_text() and "Skydnir modern" in (ROOT / "app/src/modern/res/values/strings.xml").read_text())
    ...
    require("apk build metadata is generated per build unless release env pins it", "PDOCKER_BUILD_TIME_UTC" in build_gradle_src and "DateTimeFormatter.ISO_INSTANT.format(skydnirBuildInstant)" in build_gradle_src and 'gitOutput("git", "rev-parse", "--short=12", "HEAD")' in build_gradle_src and 'buildConfigString(skydnirBuildTimeUtc)' in build_gradle_src and 'buildConfigString(skydnirBuildCommit)' in build_gradle_src and 'buildConfigString(skydnirBuildNumber)' in build_gradle_src)
    manifest_src = (ROOT / "app/src/main/AndroidManifest.xml").read_text()
    ...
    require("apk setting configures shared documents volume env", "REQUEST_DOCUMENTS_TREE" in main_src and "ACTION_OPEN_DOCUMENT_TREE" in main_src and "takePersistableUriPermission" in main_src and "PDOCKER_DOCUMENTS_HOST" in main_src and "SKYDNIR_DOCUMENTS_HOST" in main_src and "PDOCKER_DOCUMENTS_MOUNT" in main_src and "SKYDNIR_DOCUMENTS_MOUNT" in main_src and "writeDocumentsEnv" in main_src and "syncDocumentsVolumeEnv" in main_src and "applySkydnirEnvAliases" in main_src)
    ...
    require("legacy project migration note uses Skydnir public name", "LEGACY_PROJECT_MIGRATION.md" in main_src and "Skydnir copied lightweight project definition files" in main_src and "pdocker copied lightweight project definition files" not in main_src)
    ...
    require("compose parser expands project env for documents volume", "composeEnvironment(dir)" in main_src and "File(projectDir, \".env\")" in main_src and "File(projectRoot, \".pdocker-common.env\")" in main_src and "File(projectRoot, \".skydnir-common.env\")" in main_src and "composeValue(value, composeEnv)" in main_src)
    ...
    pdockerd_gpu_env_manifest_driven = "gpu_runtime_env_defaults(\"vulkan\")" in pdockerd_src and "pdockerd_runtime_env_defaults" in pdockerd_src
    require("gpu executor supports default duplicate descriptor aliasing", "PDOCKER_GPU_REWRITE_DUPLICATE_DESCRIPTOR_BINDINGS" in llama_gpu_env_manifest_src and "PDOCKER_GPU_REWRITE_DUPLICATE_DESCRIPTOR_BINDINGS" in gpu_executor_src and "env_truthy(\"PDOCKER_GPU_REWRITE_DUPLICATE_DESCRIPTOR_BINDINGS\", 1)" in gpu_executor_src and "rewrite_duplicate_descriptor_bindings" in gpu_executor_src and pdockerd_gpu_env_manifest_driven)
    require("gpu descriptor alias materialization option is propagated", "PDOCKER_GPU_MATERIALIZE_DESCRIPTOR_ALIASES" in llama_gpu_env_manifest_src and "PDOCKER_GPU_MATERIALIZE_DESCRIPTOR_ALIASES" in gpu_executor_src and "materialize_descriptor_aliases" in gpu_executor_src and "PDOCKER_GPU_VULKAN_BOOL_DISPATCH_OPTIONS(PDOCKER_VK_BOOL_BRIDGE_OPTION)" in vulkan_icd_src and pdockerd_gpu_env_manifest_driven)
    require("gpu resident cache can be disabled from tests through icd and executor", "PDOCKER_GPU_RESIDENT_CACHE" in llama_gpu_env_manifest_src and "PDOCKER_GPU_VULKAN_BOOL_DISPATCH_OPTIONS(PDOCKER_VK_BOOL_BRIDGE_OPTION)" in vulkan_icd_src and "resident_cache_candidate(options" in gpu_executor_src and pdockerd_gpu_env_manifest_driven)
    require("q6k oracle writeback diagnostic is explicitly gated and propagated", "PDOCKER_GPU_Q6K_ORACLE_WRITEBACK" in llama_gpu_env_manifest_src and "PDOCKER_GPU_VULKAN_BOOL_DISPATCH_OPTIONS(PDOCKER_VK_BOOL_BRIDGE_OPTION)" in vulkan_icd_src and "oracle_writeback_rows" in gpu_executor_src and pdockerd_gpu_env_manifest_driven)
    require("q6k safe gpu kernel is explicitly gated and propagated", "PDOCKER_GPU_Q6K_SAFE_KERNEL" in llama_gpu_env_manifest_src and "PDOCKER_GPU_VULKAN_BOOL_DISPATCH_OPTIONS(PDOCKER_VK_BOOL_BRIDGE_OPTION)" in vulkan_icd_src and "kQ6kSafeSpv" in gpu_executor_src and "q6k_safe_kernel" in gpu_executor_src and pdockerd_gpu_env_manifest_driven)
    require("gpu executor exposes an audited driver pipeline optimization toggle", "PDOCKER_GPU_DISABLE_PIPELINE_OPTIMIZATION" in llama_gpu_env_manifest_src and "PDOCKER_GPU_DISABLE_PIPELINE_OPTIMIZATION" in gpu_executor_src and "VK_PIPELINE_CREATE_DISABLE_OPTIMIZATION_BIT" in gpu_executor_src and pdockerd_gpu_env_manifest_driven)
    ...
    require("large engine request bodies stream to temp files", "pdloadbody_" in pdockerd_src and "open(body_path, \"rb\")" in pdockerd_src and "pdarchiveput_" in pdockerd_src and "safe_extract_container_archive(body_path" in pdockerd_src and "tarfile.open(tar_path" in pdockerd_src and "BytesIO(body)" not in pdockerd_src)
    ...
    require("build prune deletes stale build cache metadata", "BUILD_CACHE_DIR" in pdockerd_src and "build-cache-stale" in pdockerd_src and "not _layer_exists(did)" in pdockerd_src)
    require("build prune covers archive temp prefixes", "pdarchiveput_" in pdockerd_src and "pdbuildctx_" in pdockerd_src and "pdloadbody_" in pdockerd_src and "pdsavebody_" in pdockerd_src)
    ...
    engine_exec_session_src = (ROOT / "app/src/main/kotlin/io/github/ryo100794/pdocker/EngineExecSession.kt").read_text()
    require("engine exec supports docker-compatible tty resize", "^/exec/(.+?)/resize$" in pdockerd_src and "TIOCSWINSZ" in pdockerd_src and "SIGWINCH" in pdockerd_src and "resizeEngineExecSync" in engine_exec_session_src and "/resize?h=$rows&w=$cols" in engine_exec_session_src)
    ...
    require("release signing is externalized", 'providers.environmentVariable("SKYDNIR_${name}")' in build_gradle_src and 'providers.environmentVariable("PDOCKER_${name}")' in build_gradle_src and "SIGNING_STORE_FILE" in build_gradle_src and "release-signing.properties" in build_gradle_src and "*.jks" in gitignore_src and "*.keystore" in gitignore_src and "*.p12" in gitignore_src)
    ...
    require("service health probes are scheduled only for running http services", "urls.filter { it.serviceName in runningServices && isHttpServiceUrl(it.url) }" in main_src and "scheduleServiceHealthProbe(proof, serviceUrl.url)" in main_src)
    require("service health probes require current engine snapshot proof", "serviceTruthEngineSnapshotIsCurrent()" in main_src and "runningProofs[it.serviceName]?.engineSnapshotCurrent == true" in main_src and "!engineSnapshotCurrent -> getString(R.string.service_health_unknown)" in main_src and "proof == null || !proof.engineSnapshotCurrent" in main_src)
    require("service health cache is keyed by engine id and url", "private fun serviceHealthKey(engineContainerId: String, url: String)" in main_src and "val cacheKey = serviceHealthKey(proof.engineContainerId, url)" in main_src and "serviceHealth[serviceHealthKey(proof.engineContainerId, serviceUrl.url)]" in main_src and "scheduleServiceHealthProbe(it.url)" not in main_src)
    ...
    require("container exposes explicit engine api it terminal", "action_container_terminal_fmt" in string_src and "detail_container_terminal" in string_src and "ENGINE_EXEC_PREFIX" in main_src and "engineExecTerminalCommand" in main_src and "/exec/$execId/start" in engine_exec_session_src and 'import org.json.JSONArray' in engine_exec_session_src and "command -v /bin/bash" in engine_exec_session_src and "exec /bin/bash -i" in engine_exec_session_src and "exec /bin/sh -i" in engine_exec_session_src and '"TERM=xterm-256color"' in engine_exec_session_src and '"COLORTERM=truecolor"' in engine_exec_session_src and "docker exec -it" not in main_src)
    require("engine exec it uses docker hijack stdin without private stdin api", "PdockerOutOfBandStdin" not in bridge_src_now and "PdockerOutOfBandStdin" not in engine_exec_session_src and "PdockerOutOfBandStdin" not in pdockerd_src and "/stdin" not in bridge_src_now and "/stdin" not in engine_exec_session_src and 're.match(r"^/exec/(.+?)/stdin$"' not in pdockerd_src and "socket.outputStream.write(bytes)" in engine_exec_session_src and "_pipe_hijacked_stdin(proc)" in pdockerd_src)
    ...
    require("engine exec encodes container refs", "DockerEngineClient.encodePath(containerId)" in engine_exec_session_src and "urllib.parse.unquote(m.group(1))" in pdockerd_src)
    ...
    require("ros2 rviz template exposes noVNC and raw VNC host ports", "18082:6080" in ros2_rviz_compose and "15900:5900" in ros2_rviz_compose and "skydnir.service-url: 18082=noVNC RViz" in ros2_rviz_compose)
    ...
    require("image pull uses engine api with searchable refs and explicit platform", "showPullImageDialog" in main_src and "runPullImage(image, imagePlatform)" in main_src and "fetchDockerHubImageRefs" in main_src and "currentImagePlatform" in main_src and "abiDefaultImagePlatform" in main_src and "pullImage(image, platform)" in main_src and "pullImage(\"ubuntu:22.04\")" not in main_src and "/images/create?fromImage=" in engine_src and "platformQuery" in engine_src and "&platform=" in engine_src and "normalize_platform(query.get(\"platform\"" in pdockerd_src and "pull_image(image, platform=platform)" in pdockerd_src and "invalid platform" in pdockerd_src and "Open docker pull shell" not in string_src and "docker pull 用シェル" not in (ROOT / "app/src/main/res/values-ja/strings.xml").read_text())
    require("image pull/update wording preserves old tags until publish", "Pull/update image" in string_src and "Existing tags are updated atomically through the Engine API" in string_src and "any existing tag is kept until publish succeeds" in string_src and "pull/更新" in (ROOT / "app/src/main/res/values-ja/strings.xml").read_text() and ".old-" in pdockerd_src and "os.replace(d, backup)" in pdockerd_src and "os.replace(stage, d)" in pdockerd_src)
    ...
    require("images tab surfaces layer cache health and stale metadata", "renderImageCacheHealth" in main_src and "imageCacheHealth" in main_src and "staleBuildCacheCount" in main_src and "widget_image_cache_health" in string_src)
    ...
    require("compose up links header-declared services after successful start", "Service URL $label $url" in main_src and "openServiceWhenReady" in main_src and "ACTION_VIEW" in main_src and "parseComposeHeaderServiceLinks" in main_src and "skydnir.auto-open:" in main_src and "pdocker.auto-open:" in main_src)
    ...
    require("gpu bridge env migrates existing container state on start", "_merge_runtime_bridge_env(state)" in pdockerd_src and "PDOCKER_GPU_VIRTUAL_MEMORY" in pdockerd_src and "GGML_VK_SUBALLOCATION_BLOCK_SIZE" in llama_gpu_env_manifest_src and pdockerd_gpu_env_manifest_driven)
    ...
    repair_slice = main_src.split("private fun repairDefaultDevWorkspaceDockerfile", 1)[1].split("private fun migrateProjectPorts", 1)[0]
    require("default dev workspace repair is bounded compatibility logic", "short-lived\n    // placeholder Dockerfile template" in main_src and "knownPlaceholder" in repair_slice and 'listOf("FROM ubuntu:22.04"' in repair_slice and "echo hello from Dockerfile" in repair_slice and "composeNeedsCodeServer" in repair_slice and "if (!composeNeedsCodeServer || dockerfileProvidesCodeServer && !knownPlaceholder) return" in repair_slice and "copyTo(backup, overwrite = false)" in repair_slice)
    default_readme_src = (ROOT / "app/src/main/assets/default-project/README.md").read_text()
    default_documents_readme_src = (ROOT / "app/src/main/assets/default-project/documents/README.md").read_text()
    default_new_project_src = (ROOT / "app/src/main/assets/default-project/scripts/pdocker-new-project").read_text()
    ...
    require(
        "direct runtime probe template uses Skydnir public image and container names",
        "image: skydnir/direct-runtime-probe:latest" in direct_runtime_probe_compose_src
