...
def main() -> int:
    ...
    require("apk build metadata is generated per build unless release env pins it", "PDOCKER_BUILD_TIME_UTC" in build_gradle_src and "DateTimeFormatter.ISO_INSTANT.format(pdockerBuildInstant)" in build_gradle_src and 'gitOutput("git", "rev-parse", "--short=12", "HEAD")' in build_gradle_src and 'buildConfigString(pdockerBuildTimeUtc)' in build_gradle_src and 'buildConfigString(pdockerBuildCommit)' in build_gradle_src and 'buildConfigString(pdockerBuildNumber)' in build_gradle_src)
    manifest_src = (ROOT / "app/src/main/AndroidManifest.xml").read_text()
