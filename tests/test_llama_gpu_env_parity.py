...
class LlamaGpuEnvParityTest(unittest.TestCase):
    ...
    def test_compare_pdockerd_and_ui_compose_env_surfaces_match_manifest(self):
        ...
        for key in [
            ...
            self.assertEqual("q6_callsite_gated", config_fields[key]["evidence_policy"])
        # The compare script intentionally does not hard-code individual Q6 env
        # names; it consumes them through config_propagation_env_fields from the
        # manifest so env propagation cannot drift across surfaces again.
        for key in [
    ...
    def test_compare_echoes_and_records_manifest_based_runtime_env(self):
        ...
        self.assertIn('"process_workflow_only_env_keys": process_workflow_only_env_keys', compare)
        ...
        self.assertNotIn('effective_runtime_env.update({str(name): str(value) for name, value in planned_env.items()})', compare)
        self.assertIn('observed_runtime_env = {str(name): str(value) for name, value in runtime_env.items()}', compare)
        self.assertIn('effective_runtime_env = observed_runtime_env', compare)
        self.assertIn('requested_or_planned_env = {str(k): str(v) for k, v in manifest_requested_env.items()}', compare)
        self.assertIn('requested_or_planned_env.update({str(k): str(v) for k, v in planned_env.items()})', compare)
        self.assertIn('def requested_runtime_env_value(name):', compare)
        self.assertIn('for mapping in (intended_runtime_env, manifest_requested_env):', compare)
        self.assertIn('"planned_container_env": {str(k): str(v) for k, v in sorted(planned_env.items())}', compare)
        self.assertIn('"requested_or_planned_env": dict(sorted(requested_or_planned_env.items()))', compare)
        self.assertIn('"intended_runtime_env": dict(sorted(intended_runtime_env.items()))', compare)
        self.assertIn('"observed_runtime_env": dict(sorted(observed_runtime_env.items()))', compare)
        self.assertIn('"process_workflow_only_env_keys": runtime_process_workflow_only_env_keys', compare)
        ...
        self.assertIn('"requested_or_planned_env_observed_keys"', compare)
        self.assertIn('"requested_or_planned_env_missing_from_runtime"', compare)
        self.assertIn('"intended_env_missing_from_runtime"', compare)
        self.assertIn('"app_process_only_not_host_container_forwarded_keys"', compare)
        self.assertIn('"process_workflow_only_not_forwarded_keys"', compare)
