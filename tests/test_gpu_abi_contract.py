...
class GpuAbiContractTest(unittest.TestCase):
    ...
    def test_q6_effective_reconstructor_rejects_bitcast_without_q6_decode_consumer(self):
        ...
        self.assertEqual(step["candidate_count"], 16)
        self.assertEqual(step["reason"], "incomplete-eligible-pattern-set")
    ...
    def test_vulkan_graphics_v67_static_viewport_scissor_metadata_is_append_only(self):
        ...
        self.assertIn("FrameRange ranges[", executor)
    ...
    def test_vulkan_graphics_v61_metadata_extension_is_fail_closed_validated(self):
        ...
        range_body = header_validator.split("FrameRange ranges[", 1)[1].split(
    ...
    def test_vulkan_graphics_v61_image_barrier_replay_is_reachable(self):
        ...
        self.assertNotIn("graphics barrier batch exceeds replay stack limit", preflight_barrier_helper)
        self.assertNotIn("matched_memory_barriers", preflight_barrier_helper)
        self.assertNotIn("matched_buffer_barriers", preflight_barrier_helper)
        self.assertNotIn("matched_image_barriers", preflight_barrier_helper)
        ...
        self.assertIn("calloc(memory_barrier_capacity", recorder)
        self.assertIn("free(memory_barriers_to_record)", recorder)
        self.assertNotIn("VkMemoryBarrier2 memory_barriers_to_record[PDOCKER_GPU_MAX_VULKAN_BINDINGS]", recorder)
        self.assertNotIn("VkBufferMemoryBarrier2 buffer_barriers_to_record[PDOCKER_GPU_MAX_VULKAN_BINDINGS]", recorder)
        self.assertNotIn("VkImageMemoryBarrier2 image_barriers_to_record[PDOCKER_GPU_MAX_VULKAN_BINDINGS]", recorder)
        self.assertIn("memory_barrier_capacity", dependency_helper)
        self.assertIn("buffer_barrier_capacity", dependency_helper)
        self.assertIn("image_barrier_capacity", dependency_helper)
        self.assertNotIn("PDOCKER_GPU_MAX_VULKAN_BINDINGS", dependency_helper)
    ...
    def test_vulkan_dispatch_v5_native_plan_accepts_v5_native_table_width(self):
        ...
        self.assertNotIn("descriptor-set-exceeds-legacy-v4", body)
        ...
        self.assertIn("array-element-exceeds-v5-table-limit", body)
        self.assertIn("object-table-exceeds-v5-native", body)
        self.assertIn("plan->buffer_descriptor_count > PDOCKER_GPU_VULKAN_DISPATCH_V5_MAX_DESCRIPTORS", body)
        self.assertIn("plan->image_descriptor_count > PDOCKER_GPU_VULKAN_DISPATCH_V5_MAX_DESCRIPTORS", body)
        self.assertIn("plan->max_array_element >= PDOCKER_GPU_VULKAN_DISPATCH_V5_MAX_DESCRIPTORS", body)
        self.assertIn("plan->image_count > PDOCKER_GPU_VULKAN_DISPATCH_V5_MAX_IMAGES", body)
        self.assertIn("plan->image_view_count > PDOCKER_GPU_VULKAN_DISPATCH_V5_MAX_IMAGE_VIEWS", body)
        self.assertIn("plan->sampler_count > PDOCKER_GPU_VULKAN_DISPATCH_V5_MAX_SAMPLERS", body)
    ...
    def test_vulkan_dispatch_v5_2_icd_gates_layout_ranges_on_executor_capability(self):
        ...
        self.assertIn("executor_supports_vulkan_dispatch_v53_buffer_views", sender)
        self.assertIn("need_v53_buffer_views", sender)
    ...
    def test_vulkan_dispatch_v5_descriptor_arrays_drive_executor_layout_pool_write_and_cache(self):
        ...
        self.assertIn("set_binding_descriptor_counts[table_index] = alias_descriptor_count;", executor)
    ...
    def test_vulkan_compute_descriptor_layout_skips_sparse_binding_gaps(self):
        executor = GPU_EXECUTOR.read_text()
        ...
        self.assertIn(".descriptorCount = descriptor_count", generic_body)
        self.assertIn(".bindingCount = layout_binding_count", generic_body)
        self.assertNotIn(".bindingCount = set_layout_count", generic_body)

        pool_body = executor.split("uint32_t descriptor_pool_storage_count = 0;", 1)[1].split(
            "VkDescriptorPoolSize pool_sizes[8]", 1
        )[0]
        self.assertIn("if (descriptor_count == 0) continue;", pool_body)
        self.assertNotIn("set_binding_descriptor_counts[set_index][binding_index] ?", pool_body)
    ...
    def test_vulkan_dispatch_v5_schema_hashes_match_declared_field_macros(self):
        schemas = [
            ...
            (
                "PDOCKER_GPU_VULKAN_DISPATCH_V53_BUFFER_VIEW_FIELDS",
                "PDOCKER_GPU_VULKAN_DISPATCH_V53_BUFFER_VIEW_FIELD_COUNT",
            ...
            (
                "PDOCKER_GPU_VULKAN_GRAPHICS_V625_HEADER_EXTENSION_FIELDS",
                "PDOCKER_GPU_VULKAN_GRAPHICS_V625_HEADER_EXTENSION_FIELD_COUNT",
                "PDOCKER_GPU_VULKAN_GRAPHICS_V625_HEADER_EXTENSION_SCHEMA_HASH",
            ),
            (
    ...
    def test_vulkan_graphics_v625_descriptor_bind_metadata_abi_is_append_only(self):
        expected_extension_fields = [
            ("descriptor_bind_count", "u32"),
            ("descriptor_bind_entry_size", "u32"),
            ("descriptor_bind_table_offset", "u64"),
            ("descriptor_bind_table_size", "u64"),
            ("descriptor_bind_schema_hash", "u64"),
            ("descriptor_bind_table_hash", "u64"),
            ("extension_hash", "u64"),
        ]
        expected_bind_fields = [
            ("command_index", "u32"),
            ("first_set", "u32"),
            ("descriptor_set_count", "u32"),
            ("first_dynamic_offset", "u32"),
        ...
        for header_path in [APP_HEADER, CONTAINER_HEADER]:
            ...
            with self.subTest(header=str(header_path)):
                for header_marker in [
                    "PDOCKER_GPU_VULKAN_GRAPHICS_V625_ABI_MINOR 25u",
                    ...
                    "PDOCKER_GPU_VULKAN_GRAPHICS_V625_MAX_DESCRIPTOR_BINDS",
                    "descriptor_bind_schema_hash",
                ]:
                ...
                bind_fields, bind_count, declared_bind_hash, computed_bind_hash = vulkan_dispatch_v5_schema(
                    header_path,
                    "PDOCKER_GPU_VULKAN_GRAPHICS_V625_DESCRIPTOR_BIND_FIELDS",
                    "PDOCKER_GPU_VULKAN_GRAPHICS_V625_DESCRIPTOR_BIND_FIELD_COUNT",
                    "PDOCKER_GPU_VULKAN_GRAPHICS_V625_DESCRIPTOR_BIND_SCHEMA_HASH",
                ...
                self.assertEqual(expected_bind_fields, bind_fields)
                self.assertEqual(7, extension_count)
                self.assertEqual(7, bind_count)
                self.assertEqual(declared_extension_hash, computed_extension_hash)
                self.assertEqual(declared_bind_hash, computed_bind_hash)
                self.assertEqual(
                    [name for name, _ in expected_extension_fields],
                    c_struct_field_names(header_path, "PdockerGpuVulkanGraphicsV625HeaderExtension"),
                )
                self.assertEqual(
                    [name for name, _ in expected_bind_fields],
                    c_struct_field_names(header_path, "PdockerGpuVulkanGraphicsV625DescriptorBindEntry"),
                )
                self.assertEqual(
                    c_struct_field_names(header_path, "PdockerGpuVulkanGraphicsV624FrameHeader") + ["v625"],
                    c_struct_field_names(header_path, "PdockerGpuVulkanGraphicsV625FrameHeader"),
                )

        executor = GPU_EXECUTOR.read_text()
        for executor_marker in [
            "case PDOCKER_GPU_VULKAN_GRAPHICS_V625_ABI_MINOR: return 51u",
            "sizeof(PdockerGpuVulkanGraphicsV625FrameHeader)",
            "const PdockerGpuVulkanGraphicsV625FrameHeader *header_v625",
            "view->header_v625",
            "view->is_v625",
            "view->descriptor_binds",
            "header_v625->v625.descriptor_bind_count",
            "descriptor_bind_schema_hash != PDOCKER_GPU_VULKAN_GRAPHICS_V625_DESCRIPTOR_BIND_SCHEMA_HASH",
            "descriptor_bind_table_hash",
            "find_vulkan_graphics_v625_descriptor_bind",
            "bind_meta ? bind_meta->descriptor_set_count",
        ...
        icd = VULKAN_ICD.read_text()
        for icd_marker in [
            "PdockerGpuVulkanGraphicsV625DescriptorBindEntry descriptor_binds",
            "PdockerGpuVulkanGraphicsV625FrameHeader *frame_header_v625",
            "need_v625_descriptor_bind",
            "PDOCKER_GPU_VULKAN_GRAPHICS_V625_ABI_MINOR",
            "frame_header_v625->v625.descriptor_bind_count",
            "frame_header_v625->v625.descriptor_bind_entry_size",
            "PDOCKER_GPU_VULKAN_GRAPHICS_V625_DESCRIPTOR_BIND_SCHEMA_HASH",
            "APPEND_GRAPHICS_TABLE(descriptor_binds, descriptor_bind_count",
            "frame_header_v625->v625.descriptor_bind_table_hash",
            "VULKAN_GRAPHICS_V6.25",
    ...
    def test_vulkan_dispatch_v5_schema_is_single_source_and_advertised(self):
        ...
        for marker in [
            ...
            "PDOCKER_GPU_VULKAN_DISPATCH_V5_SAMPLER_SCHEMA_HASH",
            "PDOCKER_GPU_VULKAN_DISPATCH_V5_DESCRIPTOR_OBJECT_SCHEMA_HASH",
            "PDOCKER_GPU_VULKAN_DISPATCH_V53_ABI_MINOR",
            "PDOCKER_GPU_VULKAN_DISPATCH_V5_ABI_MINOR_BUFFER_VIEWS",
            "PDOCKER_GPU_VULKAN_DISPATCH_V53_HEADER_EXTENSION_FIELDS",
            "PDOCKER_GPU_VULKAN_DISPATCH_V53_HEADER_EXTENSION_FIELD_COUNT",
            "PDOCKER_GPU_VULKAN_DISPATCH_V53_BUFFER_VIEW_FIELDS",
            "PDOCKER_GPU_VULKAN_DISPATCH_V53_BUFFER_VIEW_FIELD_COUNT",
            "PDOCKER_GPU_VULKAN_DISPATCH_V53_BUFFER_VIEW_SCHEMA_HASH",
            "PDOCKER_GPU_VULKAN_DISPATCH_V53_MAX_BUFFER_VIEWS",
        ]:
            self.assertIn(marker, app)
            self.assertIn(marker, container)
    ...
    def test_vulkan_dispatch_reports_binding_diagnostics(self):
        ...
        self.assertIn("double *binding_upload_ms = NULL;", source)
        self.assertIn("double *binding_download_ms = NULL;", source)
        self.assertIn("size_t *binding_dirty_probe_pages = NULL;", source)
        self.assertIn("size_t *binding_dirty_probe_bytes = NULL;", source)
        self.assertIn("uint64_t *binding_fd_before_hash = NULL;", source)
        self.assertIn("uint64_t *binding_gpu_after_dispatch_hash = NULL;", source)
        self.assertIn("size_t *binding_alias_rep = NULL;", source)
        self.assertIn("uint8_t *binding_group_read_needed = NULL;", source)
        self.assertIn("off_t *binding_group_base = NULL;", source)
        self.assertIn("off_t *binding_group_end = NULL;", source)
        self.assertIn("size_t *binding_gpu_offset = NULL;", source)
        self.assertIn("PDOCKER_CALLOC_TABLE(binding_upload_ms, binding_table_capacity);", source)
        self.assertIn("PDOCKER_CALLOC_TABLE(binding_gpu_offset, binding_table_capacity);", source)
        ...
        self.assertIn("uint8_t *binding_group_span_seen = NULL;", source)
        self.assertIn("PDOCKER_CALLOC_TABLE(binding_group_span_seen, binding_table_capacity);", source)
        ...
        self.assertIn("size_t *binding_descriptor_offset = NULL;", source)
        self.assertIn("PDOCKER_CALLOC_TABLE(binding_descriptor_offset, binding_table_capacity);", source)
    ...
    def test_llama_gpu_compare_tracks_critical_env_propagation(self):
        ...
        self.assertIn("Runtime Q6 compatibility rewrites", source)
        self.assertIn("structural SPIR-V/interface checks instead of hashes", source)
    ...
    def test_vulkan_descriptor_arrays_preserve_array_elements_with_v5_transport(self):
        ...
        self.assertIn("PdockerVkDescriptorBinding **storage_buffers;", icd)
        self.assertIn("uint32_t *storage_buffer_counts;", icd)
        self.assertIn("descriptor_set_binding_slot", icd)
        self.assertNotIn("PdockerVkDescriptorBinding (*storage_buffers)[PDOCKER_VK_MAX_DESCRIPTOR_ARRAY_ELEMENTS]", icd)
        ...
        self.assertIn("descriptor_set_binding_slot_const(src, src_binding, src_array)", icd)
        self.assertIn("descriptor_set_binding_slot(dst, dst_binding, dst_array)", icd)
        self.assertNotIn("set->storage_buffers[binding][array_element]", icd)
        self.assertNotIn("src->storage_buffers[src_binding][src_array]", icd)
        self.assertNotIn("dst->storage_buffers[dst_binding][dst_array]", icd)
    ...
    def test_vulkan_immutable_samplers_preserve_descriptor_object_fidelity(self):
        ...
        self.assertIn("descriptor_set_layout_allocate_immutable_sampler_row", icd)
        self.assertNotIn("PdockerVkSampler (*immutable_samplers)[PDOCKER_VK_MAX_DESCRIPTOR_ARRAY_ELEMENTS]", icd)
        self.assertNotIn("bool (*immutable_sampler_valid)[PDOCKER_VK_MAX_DESCRIPTOR_ARRAY_ELEMENTS]", icd)
        ...
        self.assertIn("descriptor_layout_immutable_sampler_valid(actual, i, array_element)", compat_body)
        self.assertIn("descriptor_layout_immutable_sampler(expected, i, array_element)", compat_body)
    ...
    def test_vulkan_descriptor_copy_is_typed_and_fail_closed(self):
        ...
        self.assertLess(
            ...
            update_body.index("descriptor_set_binding_slot(dst, dst_binding, dst_array)"),
        )
    ...
    def test_vulkan_v4_transport_supports_uniform_buffer_descriptor_type(self):
        ...
        self.assertIn("unsupported descriptor write type for V5 transport", executor)
        run_body = c_function_body(executor, "run_vulkan_dispatch_fd")
        marker = "const uint32_t set_binding_limit = set_binding_counts[set_index];"
        generic_body = marker + run_body.split(marker, 1)[1].split(
            "fail_stage = \"create-generic-descriptor-set-layout\"", 1
    ...
    def test_vulkan_compute_descriptor_writes_are_heap_backed_for_v5_table_width(self):
        ...
        self.assertIn("size_t descriptor_alias_write_count = 0;", body)
        self.assertIn("descriptor_write_capacity = binding_count + image_descriptor_count + descriptor_alias_write_count;", body)
        self.assertIn("descriptor write count overflow", body)
    ...
    def test_vulkan_compute_dispatch_executor_work_tables_are_heap_backed_for_v5_width(self):
        executor = GPU_EXECUTOR.read_text()
        body = c_function_body(executor, "run_vulkan_dispatch_fd")
        self.assertIn("VulkanVectorBuffer *temp_buffers = NULL;", body)
        self.assertIn("VulkanVectorBuffer *alias_temp_buffers = NULL;", body)
        self.assertIn("VulkanVectorBuffer **vk_buffers = NULL;", body)
        self.assertIn("VulkanStrictMemoryObject *strict_memories = NULL;", body)
        self.assertIn("VulkanStrictBufferObject *strict_buffers = NULL;", body)
        self.assertIn("VulkanDispatchImageMemoryObject *image_memories = NULL;", body)
        self.assertIn("VulkanDispatchImageObject *dispatch_images = NULL;", body)
        self.assertIn("VulkanDispatchImageViewObject *dispatch_image_views = NULL;", body)
        self.assertIn("VulkanDispatchSamplerObject *dispatch_samplers = NULL;", body)
        self.assertIn("const size_t binding_table_capacity = binding_count ? binding_count : 1u;", body)
        self.assertIn("const size_t alias_table_capacity = PDOCKER_GPU_VULKAN_DISPATCH_V5_MAX_DESCRIPTORS;", body)
        self.assertIn("image_memory_capacity > PDOCKER_GPU_VULKAN_DISPATCH_V5_MAX_RESOURCES", body)
        self.assertIn("image_table_capacity > PDOCKER_GPU_VULKAN_DISPATCH_V5_MAX_IMAGES", body)
        self.assertIn("PDOCKER_CALLOC_TABLE(temp_buffers, binding_table_capacity);", body)
        self.assertIn("PDOCKER_CALLOC_TABLE(alias_temp_buffers, alias_table_capacity);", body)
        self.assertIn("PDOCKER_CALLOC_TABLE(binding_dirty_probe_masks, binding_table_capacity);", body)
        self.assertIn("PDOCKER_CALLOC_TABLE(binding_aliases, alias_table_capacity);", body)
        self.assertIn("binding_count > PDOCKER_GPU_VULKAN_DISPATCH_V5_MAX_DESCRIPTORS", body)
        self.assertIn("image_descriptor_count > PDOCKER_GPU_VULKAN_DISPATCH_V5_MAX_DESCRIPTORS", body)
        self.assertIn("binding_table_capacity * sizeof(strict_graph_active_bindings[0])", body)
        self.assertNotIn("binding-count-exceeds-cache-capacity", body)
        self.assertIn("adopt_strict_graph_cache_entry(", body)
        self.assertNotIn("strict_memory_count > PDOCKER_GPU_MAX_VULKAN_BINDINGS", body)
        self.assertNotIn("strict_buffer_count > PDOCKER_GPU_MAX_VULKAN_BINDINGS", body)
        self.assertIn("free(binding_fd_ino);", body)
        self.assertIn("free(temp_buffers);", body)
        self.assertNotIn("VulkanVectorBuffer temp_buffers[PDOCKER_GPU_MAX_VULKAN_BINDINGS];", body)
        self.assertNotIn("VulkanStrictMemoryObject strict_memories[PDOCKER_GPU_MAX_VULKAN_BINDINGS];", body)
        self.assertNotIn("unsigned char *binding_dirty_probe_masks[PDOCKER_GPU_MAX_VULKAN_BINDINGS];", body)
        self.assertNotIn("binding_count > PDOCKER_GPU_MAX_VULKAN_BINDINGS", body)
        self.assertNotIn("image_descriptor_count > PDOCKER_GPU_MAX_VULKAN_BINDINGS", body)
        self.assertNotIn("sizeof(strict_graph_active_bindings)", body)

    def test_vulkan_strict_graph_cache_is_heap_backed_for_v5_width(self):
        executor = GPU_EXECUTOR.read_text()
        cache_struct = executor.split("} VulkanStrictGraphCacheEntry;", 1)[0].rsplit("typedef struct {", 1)[1]
        self.assertIn("VulkanStrictMemoryObject *memories;", cache_struct)
        self.assertIn("VulkanStrictBufferObject *buffers;", cache_struct)
        self.assertNotIn("memories[PDOCKER_GPU_MAX_VULKAN_BINDINGS]", cache_struct)
        self.assertNotIn("buffers[PDOCKER_GPU_MAX_VULKAN_BINDINGS]", cache_struct)

        key_body = c_function_body(executor, "strict_graph_cache_key")
        self.assertIn("const size_t memory_capacity = binding_count ? binding_count : 1u;", key_body)
        self.assertIn("uint64_t *byte_memory_ids = NULL;", key_body)
        self.assertIn("size_t *byte_memory_sizes = NULL;", key_body)
        self.assertIn("byte_memory_ids = calloc(memory_capacity, sizeof(*byte_memory_ids));", key_body)
        self.assertIn("byte_memory_sizes = calloc(memory_capacity, sizeof(*byte_memory_sizes));", key_body)
        self.assertIn("u64 = (uint64_t)i; hash = fnv1a64_update(hash, &u64, sizeof(u64));", key_body)
        self.assertIn("strict-graph-cache-oom", key_body)
        self.assertIn("free(byte_memory_ids);", key_body)
        self.assertIn("free(byte_memory_sizes);", key_body)
        self.assertNotIn("byte_memory_ids[PDOCKER_GPU_MAX_VULKAN_BINDINGS]", key_body)
        self.assertNotIn("byte_memory_count < PDOCKER_GPU_MAX_VULKAN_BINDINGS", key_body)

        destroy_body = c_function_body(executor, "destroy_strict_graph_cache_entry")
        self.assertIn("VulkanStrictMemoryObject *memories = entry->memories;", destroy_body)
        self.assertIn("VulkanStrictBufferObject *buffers = entry->buffers;", destroy_body)
        self.assertIn("free(memories);", destroy_body)
        self.assertIn("free(buffers);", destroy_body)

        adopt_body = c_function_body(executor, "adopt_strict_graph_cache_entry")
        self.assertIn("if (slot->valid)", adopt_body)
        self.assertIn("destroy_strict_graph_cache_entry(slot->device, slot);", adopt_body)
        self.assertIn("calloc(memory_count, sizeof(*slot_memories))", adopt_body)
        self.assertIn("calloc(buffer_count, sizeof(*slot_buffers))", adopt_body)
        self.assertIn("slot->memories = slot_memories;", adopt_body)
        self.assertIn("slot->buffers = slot_buffers;", adopt_body)
        self.assertIn("attach_strict_graph_views(slot->memories, slot->buffers, slot->buffer_count);", adopt_body)

        rewrite_body = c_function_body(executor, "rewrite_duplicate_descriptor_bindings")
        self.assertIn("uint32_t binding_capacity", executor)
        self.assertIn("size_t alias_capacity", executor)
        self.assertIn("const uint32_t descriptor_set_capacity = PDOCKER_GPU_MAX_VULKAN_DESCRIPTOR_SETS;", rewrite_body)
        self.assertIn("descriptor_slot_capacity =", rewrite_body)
        self.assertIn("used = (uint8_t *)calloc(descriptor_slot_capacity, sizeof(uint8_t));", rewrite_body)
        self.assertIn("first_seen = (uint8_t *)calloc(descriptor_slot_capacity, sizeof(uint8_t));", rewrite_body)
        self.assertIn("(size_t)bindings[i].descriptor_set * binding_capacity + bindings[i].binding", rewrite_body)
        self.assertIn("candidate < binding_capacity", rewrite_body)
        self.assertIn("(size_t)descriptor_set * binding_capacity + candidate", rewrite_body)
        self.assertIn("alias_used >= alias_capacity", rewrite_body)
        self.assertNotIn("uint8_t used[PDOCKER_GPU_MAX_VULKAN_BINDINGS]", rewrite_body)

        image_body = c_function_body(executor, "materialize_vulkan_dispatch_images")
        self.assertIn("size_t memory_capacity", executor)
        self.assertIn("size_t image_capacity", executor)
        self.assertIn("size_t view_capacity", executor)
        self.assertIn("size_t sampler_capacity", executor)
        self.assertIn("object_tables->image_count > image_capacity", image_body)
        self.assertNotIn("object_tables->image_count > PDOCKER_GPU_MAX_VULKAN_BINDINGS", image_body)

        strict_body = c_function_body(executor, "create_strict_vulkan_object_graph")
        self.assertIn("size_t memory_capacity", executor)
        self.assertIn("size_t buffer_capacity", executor)
        self.assertIn("*memory_count >= memory_capacity", strict_body)
        self.assertIn("*buffer_count >= buffer_capacity", strict_body)
    ...
    def test_vulkan_descriptor_set_layout_support_is_mapped_and_conservative(self):
        ...
        self.assertIn("binding->descriptorCount > PDOCKER_GPU_VULKAN_DISPATCH_V5_MAX_DESCRIPTORS", helper_body)
    ...
    def test_vulkan_buffer_view_state_is_tracked_but_texel_submit_remains_abi_gated(self):
        ...
        self.assertIn("if (texel_size == 0)", create_body)
        ...
        self.assertIn("api_buffer_view_ids", icd)
        self.assertIn("api_buffer_view_formats", icd)
        self.assertIn("api_buffer_view_offsets", icd)
        ...
        self.assertIn("executor_supports_vulkan_dispatch_v53_buffer_views();", icd)
        self.assertIn("VK_FORMAT_FEATURE_UNIFORM_TEXEL_BUFFER_BIT", icd)
        self.assertIn("VK_FORMAT_FEATURE_STORAGE_TEXEL_BUFFER_BIT", icd)
        self.assertIn("pProperties->limits.maxTexelBufferElements", icd)
        self.assertIn("pProperties->limits.minTexelBufferOffsetAlignment", icd)
        self.assertIn("buffer-view-format-feature-unsupported", icd)
        self.assertIn("buffer-view-offset-alignment-invalid", icd)
        self.assertIn("buffer-view-texel-count-exceeds-limit", icd)
        self.assertNotIn("texel-buffer descriptor ABI pending", icd)

        executor = GPU_EXECUTOR.read_text()
        self.assertIn("VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER", executor)
        self.assertIn("VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER", executor)
        self.assertIn("vulkan_dispatch_descriptor_type_requires_buffer_view", executor)
    ...
    def test_vulkan_duplicate_binding_rewrite_avoids_passed_bindings(self):
        ...
        self.assertIn("const uint32_t descriptor_set_capacity = PDOCKER_GPU_MAX_VULKAN_DESCRIPTOR_SETS;", source)
        self.assertIn("descriptor_slot_capacity", source)
        self.assertIn("used[slot] = 1;", source)
        self.assertIn("descriptor_sets[code[i + 1]] = code[i + 3];", source)
        self.assertIn("has_descriptor_set[code[i + 1]] = 1;", source)
        self.assertIn("const uint32_t descriptor_set = descriptor_sets[code[i + 1]];", source)
        self.assertIn("aliases[alias_used].descriptor_set = descriptor_set;", source)
        self.assertNotIn("descriptor_sets[code[i + 1]] != 0", source)
        self.assertNotIn("duplicate descriptor rewrite is single descriptor set only", source)
        self.assertNotIn("descriptor alias rewrite multi-set pending", source)
    ...
    def test_vulkan_dispatch_can_skip_unused_descriptor_transfers(self):
        ...
        self.assertIn("uint8_t *active_bindings = NULL;", source)
        self.assertIn("uint8_t *binding_read_needed = NULL;", source)
        self.assertIn("uint8_t *binding_write_needed = NULL;", source)
        self.assertIn("PDOCKER_CALLOC_TABLE(active_bindings, binding_table_capacity);", source)
    ...
    def test_llama_compare_records_bisection_and_config_propagation(self):
        ...
        self.assertIn("if (q6k_safe_kernel_requested && q6_structural_callsite_detected)", source)
        self.assertNotIn("q6k_safe_kernel_requested && is_q6k_matvec_hash", source)
    ...
    def test_llama_gpu_compare_memory_preflight_uses_available_memory(self):
        ...
        self.assertIn("q6_native_callsite_detected =\n        !q6k_safe_kernel_requested &&\n        q6_structural_callsite_detected", executor)
        ...
        self.assertNotIn("is_q6k_matvec_hash(q6_storage16_lowering_identity_hash)", executor)
        self.assertNotIn("source_spirv_hash,\n        size_t *lowered_count", executor)
        self.assertIn('\\"q6_probe_effective_replay\\":%s', executor)
        self.assertIn("find_q6k_duplicate_binding0_views", executor)
        self.assertNotIn("Q6_STORAGE16_EXPECTED_LOADS", executor)
        self.assertNotIn("Q6_U32_TO_U8VEC4_EXPECTED_BITCASTS", executor)
        self.assertIn("pattern_count != candidate_count", executor)
        ...
        self.assertNotIn("Q6_EXPECTED_FINAL_OUTPUT_STORES", final_store_rewrite)
        ...
        self.assertIn("final_output_store_count == 0", final_store_rewrite)
